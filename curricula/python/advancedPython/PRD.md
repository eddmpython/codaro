...
### 실행 환경
**⚠️ Codaro 로컬 Python 환경** - 로컬에서 실행되는 Python

### Codaro 로컬 Python 작성 기준
| 주제 | 기준 |
|-------|-----|
| asyncio | 이벤트 루프 동작과 비동기 함수 구조를 직접 다룬다 |
| 멀티스레딩/프로세싱 | 고급 주제로 분리하고 종료 절차와 플랫폼 차이를 설명한다 |
| 파일 시스템 | tempfile과 pathlib로 안전한 로컬 경로에서 실습한다 |
| C 확장 패키지 | 프로젝트 의존성으로 준비하고 설치 실패 시 대체 경로를 적는다 |
| subprocess | 실제 명령 실행은 영향 범위를 명시하고 읽기 전용 예제로 시작한다 |
| 외부 패키지 설치 | 레슨 밖에서 uv로 준비하고 레슨 안에서는 import와 사용법에 집중한다 |
| 네트워크 요청 | urllib.request.urlopen과 예외 처리를 기본으로 한다 |
| 디버깅 | traceback, logging, breakpoint 사용 맥락을 함께 다룬다 |
...
### Codaro 규칙 (30days와 동일)
1. **yaml 1개 = Codaro 노트북 1개**
2. **type: code 1개 = Codaro 셀 1개**
3. **변수명 재할당 금지** (다른 셀에서 같은 변수명 사용 불가)
4. **상수는 최상단에서만 정의**
5. **print() 사용 금지** (마지막 표현식 자동 출력)

### Codaro 로컬 Python 작성 규칙
1. **실행 범위 명시** - 파일, 네트워크, subprocess는 영향 범위와 정리 절차를 함께 작성
2. **안전한 로컬 I/O** - tempfile과 pathlib 중심으로 실습하고 실제 프로젝트 파일 삭제를 피함
3. **비동기/동시성은 심화로 분리** - 이벤트 루프, 스레드, 프로세스의 종료 조건을 함께 설명
4. **표준 라이브러리 우선** - 외부 패키지는 uv로 준비된 상태에서 import 확인부터 시작
...
1. **Codaro 로컬 Python 호환**: 모든 코드가 로컬 커널에서 재현 가능
2. **완전 독립**: 모든 섹션이 어떤 순서로 실행되어도 작동
3. **심도 있는 설명**: 개념의 why, what, how 모두 다룸
4. **Codaro 최적화**: 표현식 자동 출력 100% 활용
5. **품질 보증**: 모든 검증 체크 통과
