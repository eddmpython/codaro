...
학습카드의 `check` 필드는 학생이 연습 셀을 실행한 직후 같은 pipeline에서 pass/fail과 다음 수정 근거를 자동 계산하는 내부 채점 계약이다. 별도 `검증하기`, `완료`, `제출`, hint reveal 버튼을 요구하지 않는다. `check.type`이 비어 있으면 strong evidence를 만들 수 없으므로 새 레슨은 실행 가능한 structured CheckSpec을 제공해야 한다.
...
2. **설명용 key** - 평가에 영향 없음. 필요한 경우 연습 목표와 자동 결과 맥락에 학습자 언어로 사용한다. 내부 채점 메타를 별도 "확인" 패널로 덤프하지 않는다. 권장 어휘:
