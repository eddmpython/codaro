...
export function CheckResultPanel({
  ...
  return (
    <div
      className={cn(
        "space-y-2 border-l-2 py-1.5 pl-4 pr-2 text-sm text-foreground",
        passed ? "border-success bg-success/10" : "border-destructive",
      )}
...
function CreditNotice({
  ...
  return (
    <div
      className="flex flex-col gap-1 rounded border bg-background/60 px-2 py-2 text-[11px] leading-5 text-foreground"
      ...
      <ul className="flex flex-wrap gap-1">
        {credited.map((outcomeId) => (
          <li key={outcomeId} className="flex items-center gap-1">
            {autoSet.has(outcomeId) ? <Trophy className="size-3 text-muted-foreground" /> : null}
...
function PredictionDiffPanel({ diff }: { diff: PredictionDiffPayload }) {
  ...
  return (
    <div
      className={cn(
        "space-y-1.5 rounded border bg-background/60 px-2 py-2",
        allMatch ? "border-border" : "border-destructive/40",
      )}
        ...
        {visibleFields.map((field) => (
          <div
            ...
            className={cn(
              ...
              field.status === "match" ? "border-border" : "border-destructive/40",
            )}
          ...
          >
            ...
            <div className="mt-0.5 grid grid-cols-2 gap-1 font-mono text-[10px]">
              <span className="text-muted-foreground">예측: {field.expected || "—"}</span>
              <span
                className={cn(
                  field.status === "match" ? "text-foreground" : "text-destructive",
                )}
              >
                실측: {field.actual || "—"}
              </span>
    ...
    <div className="space-y-2" data-check-result-misconceptions="true">
      ...
      {matches.map((match) => (
        <div
          ...
          className="border-l-2 border-warning py-1 pl-3 text-foreground"
        ...
        >
          ...
          {match.correction.hint ? (
            <div className="mt-2 flex items-start gap-1.5 text-[11px] leading-5 text-foreground/80">
              <Lightbulb className="mt-0.5 size-3 text-warning" />
  ...
  return (
    <div
      className={cn(
        ...
        tone === "expected" ? "border-border" : "border-destructive/40",
      )}
