...
- **학습** - 순수 학습 공간. `curricula/` YAML과 gap을 채우는 개인 YAML을 학습 셀 카드로 펼친다.
- **노트북** - 빈 노트북에서 시작하는 실행/편집 표면. 기본 셀은 Python 셀과 Markdown 셀이고, 특수 셀은 메타데이터로 표현한다.
...
- `editor/`는 제품 프론트 폴더명이다. 사용자에게 보이는 표면 이름은 대화, 학습, 노트북, 자동화다.
...
- [publication-compiler](architecture/publication-compiler.md)
- [static-publication](architecture/static-publication.md)
