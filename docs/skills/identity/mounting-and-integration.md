...
- 공개 import는 `from codaro import createServerApp`이다. `createServerApp()`은 독립 실행 가능하면서 동시에 다른 서버에 마운팅 가능하다.
...
- host server 마운팅은 전체 Codaro 제품 API를 붙이는 경로다. 선택 기능만 넣는 부분 임베딩은 `codaro.publication`의 block embed owner를 사용하며 server mount와 혼합하지 않는다.
- GUI, HTTP API와 Python API는 같은 compiler, builder, verifier와 publication workbench를 호출한다. 각 진입점이 target 판정이나 rollback 규칙을 복제하지 않는다.

## 관련

- [[execution-engine]] - 엔진 추상화
- [[architecture-overview]] - transport 레이어
- [[python-product-journey]] - 공개 Python API와 publication 전체 여정
- [[block-embedding]] - 선택 기능 블록의 부분 임베딩 계약
