...
2. **YAML 작성** — `meta`, `intro`, `sections` 구조의 curriculum YAML을 먼저 만든다. 신규 레슨은 `intro.diagram.steps`, `intro.diagram.runtime`, `sections[].title/subtitle/goal/why/explanation/tips/snippet/exercise/check` 계약을 쓴다. `sections[].blocks[]`는 기존 curriculum 변환용이다.
3. **커리큘럼 전개** — `write-curriculum-yaml`을 호출해 YAML을 `yamlToDocument` 변환기로 보내고, 결과 document를 커리큘럼 학습 셀로 로드한다.
4. **라이브러리 확인** — 실행할 셀에 외부 패키지가 필요하면 `packages-check`를 먼저 호출하고, 누락된 항목만 `packages-install`로 uv 설치한다.
5. **셀 단위 수정** — 추가 설명, 빈칸, 예측, 체크 셀은 `write-cell`로 한 셀씩 삽입/수정/삭제한다.
6. **셀 단위 호출** — 실행과 검증은 `cell-call`을 기본으로 한다. 하위 호환이 필요할 때만 `execute-reactive`, `check-exercise`를 직접 쓴다.
7. **진행 기록** — 주제 단위 완료, 반복 실패, 숙련도 변화는 `track-achievement`로 남긴다.
