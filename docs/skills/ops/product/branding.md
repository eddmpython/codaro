...
새 설명용 시각물과 도식은 imagegen으로 제작한다. 생성 도구의 사용 절차는 전역 imagegen 스킬을
따르고, 아래는 Codaro 수업에서 유지할 디자인 원칙이다. 실제 화면과 실행 결과는 기존 캡처 절차를
따른다. 생성한 그림을 실행 증거처럼 표시하지 않는다.

- 질감은 매끈한 무광 종이와 절제된 평면 일러스트로 통일한다. 거친 종이 결, 수채화 번짐, 유광 3D,
  금속 재질, 네온 광원과 사진풍을 섹션마다 섞지 않는다. 그림자는 형태를 구분할 때만 얕게 쓴다.
- 배경과 기본 선은 제품의 neutral 토큰, 핵심 동작은 brand accent 토큰을 기준으로 한다.
  구체적인 색 값은 `assets/brand/designSystem/tokens.json`에서 가져오며 별도 팔레트를 만들지 않는다.
  색만으로 의미를 구분하지 않고 위치, 형태와 짧은 라벨을 함께 쓴다.
- 한 장은 하나의 학습 질문에 답한다. 입력과 결과 또는 바뀌는 부분이 즉시 구분되게 하고,
  같은 수업에서는 시점, 선 굵기, 여백, 사물의 단순화 수준과 읽는 방향을 유지한다.
- 정확한 코드, 긴 설명, 정답 전체는 그림에 굽지 않는다. 설명과 라벨은 가능하면 본문에 두고,
  그림 속 숫자나 자료가 필요하면 실제 예제와 대조한다. 정답을 이미지로 먼저 노출하지 않는다.
- 프롬프트에는 섹션의 학습 질문, 표현할 대상·입력·변화, 공통 질감, 토큰에서 읽은 색,
  구도, 넣지 않을 요소를 적는다. 같은 과정의 승인된 원본을 스타일 참조로 사용하고
  `promptPath`와 `promptHash`는 기존 manifest 계약으로 기록한다.
- 검수자는 그림만 보고 무엇이 달라지는지 설명할 수 있어야 한다. 1440px, 900px, 390px에서
  본문과 함께 직접 보고 글자 가독성, 잘림, 명암과 주변 섹션의 질감 일치를 확인한다.
  학습 의미와 디자인 중 하나라도 맞지 않으면 프롬프트를 고쳐 다시 생성한다.

기존 공유 화면은 출처와 원본 질감을 보존한다. 생성 일러스트의 질감을 맞춘다는 이유로 실제
화면을 다시 그리지 않는다. 기존 누락 이미지의 보강 시점과 섹션 순서는 학습 경험 정본을 따른다.
