...
**셀은 무조건 깔끔하게 유지한다. 이 원칙은 변경 불가다.** marimo와 Colab처럼 코드와 실행
결과에 집중하도록 만든 의도적인 제품 결정이다. 기능의 필요성, 편의성, 발견 가능성, 구현 난이도,
테스트 통과를 근거로 이 원칙을 완화하거나 예외를 만들 수 없다.

- 셀 본문은 코드 또는 Markdown, 실행 결과만 소유한다. 줄 번호, 코드 선택, 오류 위치처럼
  편집 내용과 직접 결합된 표시는 편집기 안에서 처리한다.
- 빈 셀, 선택한 셀, 입력 중인 셀, 실행한 셀 모두에 적용한다. 포커스하거나 코드를 입력했다는
  이유로 보조 기능 줄을 펼치지 않는다.
- 정의, 참조, 인자, 문서, 문법 검사, 이름 변경, 수정 요청, 되돌리기 같은 보조 기능의 버튼열,
  폼, 안내문, 접힌 상세 영역, 분석 결과 패널을 셀 안이나 셀 바로 아래에 붙이지 않는다.
  기본 접힘, hover 전용, 선택 시 표시라는 이유로 별도 행이나 여백을 예약하는 것도 금지한다.
- 기능을 추가하려면 먼저 적절한 자리를 정한다. 기존 셀 바깥의 최소 실행·더보기 조작,
  문서 수준 도구, 단축키, 문맥 메뉴를 진입점으로 사용하고 입력과 분석 결과는 별도 팝업이나
  도구 패널에 둔다. 새 상시 버튼이나 도구 모음으로 셀을 확장하지 않는다.
- 보조 기능을 열고 닫아도 셀의 높이, 간격과 코드 입력 공간이 바뀌면 안 된다. 도구를 닫으면
  편집 위치로 돌아가며, 새 기능을 위해 기존 셀의 간결함을 희생하지 않는다.
- 이 경계를 어긴 변경은 제품 결함이며 완료·커밋·배포 대상으로 인정하지 않는다. 데스크톱과
  모바일에서 빈 셀, 입력 후, 도구 사용 중과 닫은 뒤의 실제 화면을 확인한다. 검사도 셀 본문의
  구성과 크기 보존을 검증해야 하며, 잘못 붙인 기능의 존재를 통과 조건으로 고정하지 않는다.

이 규칙은 운영자의 명시적인 제품 결정이다. 이후 작업자가 임의로 수정·삭제하거나 하위 문서,
기획, 구현 편의를 통해 우회할 수 없다.
...
학습에 도움이 되는지 판단하는 최우선 원칙과 엑셀 표현 예시도 같은 공통 정본에서 읽는다.
제작할 때 사용한 공통 정본의 Git 커밋, 생성 도구와 선택 이유는 프롬프트 파일에 함께 기록한다.
그 파일과 실제 바이트의 해시는 기존 `provenance.promptPath`, `provenance.promptHash`로 연결한다.
manifest에 스키마가 정의하지 않은 생성 기록 필드를 추가하지 않는다. 승인한 원본과 후처리한 발행본을
