...
브라우저 OS 방향에서 서버는 TCP 포트를 여는 것이 아니라 브라우저 커널 안 Python 앱을 요청/응답 인터페이스로 dispatch하는 능력이다.

- `pyproc-asgi-browser`는 editor build에서 실제 pyproc을 boot하고, `AsgiServer`로 Python ASGI 앱을 소켓 없이 호출한다.
- gate는 `POST /codaro/pyproc-asgi?value=41` 요청의 method, path, query, body, `browser-os-server` header가 Python 앱까지 도달했는지 확인한다.
- 응답의 status, `x-codaro-runtime: pyproc-asgi`, JSON body, body byte length를 `output/test-runner/pyproc-asgi-browser/pyproc-asgi-report.json`에 남긴다.
- 이 gate가 실패하면 Codaro의 웹 파이썬은 셀 실행과 파일 세계까지만 증명된 것이고, 브라우저 안 서버 primitive는 제품 품질 판단에 올릴 수 없다.
...
`curriculum-top-tier-audit`는 위 matrix를 통과한 뒤에도 남는 완성도 격차를 본다. 작성·의존성·structured source뿐 아니라 strong `CheckSpec`, weak-only 잔존, transfer와 retrieval assessment를 필수 영역으로 검사한다. 독립 평가 승인은 `assessment-authoring-quality`와 같은 `assessment.authoring` metadata 계약으로 검증한다. `output/test-runner/curriculum-top-tier-audit/curriculum-top-tier-report.json`의 `score`는 `scoreKind: audit-requirement-coverage`인 감사 요구사항 커버리지이며 학습 효과 점수가 아니다. report는 `curriculumQualityScore: null`을 유지하고 모든 필수 영역과 90% 이상의 독립 평가 승인 커버리지를 통과한 경우에만 콘텐츠 구현의 `topTierEligible: true`, `completionEligible: true`를 남긴다. 사람 대상 E0-E3 학습 효과와 경로 공개 승격은 별도 gate가 소유한다.
...
`diagnostic-summary-contract`는 local diagnostic summary의 category/action/redaction 계약을 고정한다. `architecture-boundary`는 report가 없는 빠른 집중 gate지만 `quality-cycle` command log path/size/freshness로 현재 커밋에서 실행됐는지 확인한다. `product-quality-audit`의 10점 값은 제품 품질 점수가 아니라 `scoreKind: wiring-coverage`인 구조 커버리지이며 `completionEligible: false`를 남긴다. `product-quality-audit/product-quality-report.json`, `design-system-contract/design-system-report.json`, `visual-accessibility-browser/visual-accessibility-report.json`, `run-local-state-browser/run-local-state-report.json`, `product-experience-browser/product-experience-report.json`, `astryx-journey/astryx-journey-report.json`의 freshness와 `gitHead`를 sequence에서 대조한다. `editor-build`와 `launcher-check`는 집중 확인 gate로 유지한다. 첫 사용자 완주 audit은 `dogfood-alpha-audit/dogfood-alpha-report.json`, 자동화 IDE audit은 `automation-ide-audit/automation-ide-report.json`, 진단/운영 표면은 `diagnostic-summary-contract/diagnostic-summary-report.json`의 fresh 여부와 `gitHead`로 대조한다. `quality-cycle`에서는 editor build 증거를 `learning-system-readiness`의 `learning-card-contract` probe, `pyproc-assets-browser/pyproc-assets-report.json`, `pyproc-runtime-fs-browser/pyproc-runtime-fs-report.json`, `pyproc-asgi-browser/pyproc-asgi-report.json`, `frontend-performance-budget`로 본다. 첫 사용자 화면은 `onboarding-browser/onboarding-report.json`, provider 설정은 `provider-settings-browser/provider-settings-report.json`, runtime 복구 UX는 `runtime-recovery-browser/runtime-recovery-report.json`으로 대조한다. 학습 품질은 `curriculum-quality-matrix/curriculum-quality-report.json`, `curriculum-top-tier-audit/curriculum-top-tier-report.json`, `playwright-curriculum-runtime/playwright-curriculum-runtime-report.json`으로 본다. launcher 설치/실행 smoke는 `install-launcher-smoke/install-launcher-report.json`, launcher check는 `install-launcher-smoke`와 `launcher-test`로 본다. `teacher-eval`, `teacher-e2e`, `assistant-workloop-contract`, `editor-runtime-preflight`, `learning-card-contract`, `learning-card-browser`는 `learning-system-readiness`의 blocking probe로 실행된다.

## 완료 판단

목표 완료 선언은 “잘 만들어졌다”는 품질 판단을 증명해야 한다. `quality-cycle`, `product-quality-audit`, `automation-ide-audit`, 개별 gate 결과가 tracked worktree clean 상태에서 최신 `output/test-runner/quality-cycle/sequence-summary.json`과 `output/test-runner/ai-live-smoke/live-smoke-report.json`을 현재 `gitHead`와 대조해 stale artifact가 아님을 확인해야 한다. 이 최종 artifact 검사는 40개 product gate 통과, 각 gate command log path/size/freshness와 실제 repo-local log 파일 존재/크기, `softFailureCount: 0`, live provider credential, clarification-before-provider, `resolve-learning-goal → search-curricula → compose-master-plan` 추천·조합, gap evidence 뒤 `packages-check → write-curriculum-yaml`, `packages-check → cell-call` exact sequence를 함께 본다. `product-quality-audit` wiring coverage 10/10은 이 완료 선언을 대체하지 않는다. live provider credential이 없는 환경에서는 `ai-live-smoke`의 `live credential missing`과 quality-cycle `softFailureCount`를 증거로 남기되, 실제 provider 품질 판단은 credential이 있는 환경에서 다시 실행한 결과를 붙인다. `service-readiness-audit`는 이전 자동화를 위한 호환 alias일 뿐 완료 기준의 이름으로 쓰지 않는다.
