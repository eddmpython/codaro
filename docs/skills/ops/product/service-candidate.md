...
`curriculum-top-tier-audit`는 위 matrix를 통과한 뒤에도 남는 완성도 격차를 본다. 작성·의존성·structured source뿐 아니라 strong `CheckSpec`, weak-only 잔존, transfer와 retrieval assessment를 필수 영역으로 검사한다. `output/test-runner/curriculum-top-tier-audit/curriculum-top-tier-report.json`의 `score`는 `scoreKind: audit-requirement-coverage`인 감사 요구사항 커버리지이며 학습 효과 점수가 아니다. report는 `curriculumQualityScore: null`, `completionEligible: false`를 유지하고 모든 필수 영역을 통과한 경우에만 `topTierEligible: true`를 남긴다.
