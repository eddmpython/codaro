...
- `diagnostic-summary-contract`는 local diagnostic summary의 최소 제품 계약이다. provider, runtime, package, frontend failure를 한 raw error로 합치지 않고 category/count/action으로 분리하며, `token`, `apiKey`, `secret`, `Authorization`, OAuth access/refresh token, `sk-...` 값이 summary/detail/metadata에 남지 않는지 확인한다.
