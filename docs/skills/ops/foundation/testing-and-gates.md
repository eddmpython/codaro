...
- `curriculum-quality-matrix`는 pandas 하나가 아니라 Python 기초, 파일 처리, 데이터 분석, 시각화, 웹 자동화 대표 주제를 실제 `yamlToDocument`로 materialize한다. `contractGapCount`가 0이 아니거나 섹션 흐름이 `section → explanation → snippet → exercise → check`를 벗어나면 실패한다. 이 gate는 `output/test-runner/curriculum-quality-matrix/curriculum-quality-report.json`에 대표 주제별 count와 `allRequiredFlowsObserved`, `allSolutionsCaptured`, `totalContractGaps`를 남기고, `quality-cycle` summary는 이 report를 `payloadGitHead` evidence로 대조한다.
