...
- `architecture-boundary`는 backend 전체 테스트에 묻히면 안 되는 아키텍처 집중 gate다. `tests/testArchitectureLayerContract.py`는 package import 방향을 보고, `tests/testTransportBoundary.py`는 router/domain 경계, compatibility shim, document/runtime/provider 경계가 두꺼워지지 않는지 본다.
