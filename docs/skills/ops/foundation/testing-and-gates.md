...
| `astryx-journey` | surface | Landing과 editor를 현재 source로 다시 build한 뒤 Landing·Learn catalog·public lesson·Web 학습·대화·노트북·자동화와 Local 학습·노트북·자동화 대표 14개 case를 Dark·Light에서 각각 확인한다. |
...
`quality-cycle`은 제품이 잘 만들어졌는지 보는 반복 검증 단위다. 순서는 `root-clean` → `docs` → `backend` → `architecture-boundary` → `design-system-contract` → `visual-assets` → `learning-method` → `learning-evidence-contract` → `learning-efficacy-report` → `learning-system-readiness` → `dogfood-alpha-audit` → `product-quality-audit` → `automation-ide-audit` → `diagnostic-summary-contract` → `ai-live-smoke` → `provider-settings-browser` → `install-launcher-smoke` → `runtime-recovery-contract` → `runtime-recovery-browser` → `pyproc-assets-browser` → `pyproc-runtime-fs-browser` → `pyproc-asgi-browser` → `curriculum-quality-matrix` → `learning-content` → `repository-simplification` → `curriculum-executability` → `curriculum-top-tier-audit` → `playwright-curriculum-runtime` → `onboarding-browser` → `web-learning` → `landing-public` → `local-studio-browser` → `product-experience-browser` → `astryx-journey` → `frontend-performance-budget` → `landing-build` → `launcher-test`다. 이 명령은 완료 선언을 대신하지 않고, provider, 학습, 자동화, runtime, Astryx 공용 디자인, 대표 실제 화면, 웹 파이썬 자산·파일·서버, 설치/런처, 온보딩, 프론트 성능, architecture-boundary가 한 사이클에서 함께 버티는지 확인한다. 묶음 실행이 끝나면 runner는 통과한 gate 수, soft failure 수, gate별 duration summary, gate별 command log path/size/freshness, 현재 `gitHead`, `startedAt`/`completedAt`, 그리고 gate별 artifact freshness를 `output/test-runner/quality-cycle/sequence-summary.json`에 남긴다. `product-quality-audit`, `design-system-contract`, `product-experience-browser`, `astryx-journey`를 포함해 report를 쓰는 gate는 summary 안에 artifact path, fresh 여부, `payloadGitHead`, `gitHeadMatches`, `payloadStatus`가 함께 들어가야 하며 report의 git head가 sequence head와 다르면 artifact failure로 sequence를 실패시킨다. `product-quality-audit`의 10점은 `scoreKind: wiring-coverage`이며 `completionEligible: false`이므로 제품 100점이나 완료 선언으로 쓰지 않는다. 나머지 browser/runtime/curriculum artifact 계약과 live credential soft/hard failure 규칙은 각 개별 gate 설명을 따른다. 이 summary는 제품 SSOT가 아니라 사람이 읽는 완료 증거다.

`product-release`는 별도 sequence다. machine gate와 Local·Web·Landing browser, launcher, 자동화, 제거 negative contract를 다시 실행한 뒤 `path-learning-signal`과 `plan-quality`를 호출한다. E2 사람 근거나 R10 독립 평가가 없으면 하위 실패를 성공으로 바꾸지 않고 `output/test-runner/product-release/sequence-summary.json`에 멈춘 gate를 기록한다. `path-efficacy-confirmatory`는 shell 배포와 분리된 featured path E3 승격 gate다.
...
- `product-experience-browser`는 landing/editor를 새로 build한 뒤 정적 Web 서버와 실제 Local API 서버를 함께 띄운다. 현재 정의된 66개 Chromium case는 390·900·1024·1440 대표 viewport에서 Landing Home·Learn catalog, Web/Local 목표 중심 학습 홈, Web Lesson·Chat·Run·Automation, Web-to-Local evidence handoff, Local Lesson·Chat·Run·Automation의 가로 overflow, control 겹침, console/asset error, 깨진 image, accessible name, Astryx theme scope와 runtime tier를 검사하며 현재 source에서 66/66, failure 0으로 통과했다. Day 1 mastery·자동 transfer·24시간 delayed retrieval, Day 2·11·15·19·20·22·27·30 다중 입력 mastery와 자동 transfer 해제, Seaborn CSV table·GIF image semantic artifact capstone, pathlib 4개, zip 4개, schedule 4개 Web flow는 실패 attempt의 progress 0을 확인한 뒤 코드를 수정하고, main 커널과 분리된 pyproc Worker의 fixture/hash/timeout 검증을 통과한 `verifiedStrongChecks` cache와 IndexedDB append-only event가 reload 뒤 유지되지만 `completedAt`은 생기지 않는지 검사한다. Day 19는 fixture 파일을, Day 20은 예상 예외를 실제 Worker에서 확인한다. transfer는 mastery event 전에는 숨고 저장 직후 자동 렌더되며, retrieval은 due 전에는 숨고 24시간 뒤 자동 렌더된 뒤 통과 event 저장 시 queue에서 빠진다. Local 학습 사례는 Local 커널의 성공 상태 `done` 뒤 같은 CheckSpec을 native Python `local-sandbox`에서 다시 실행하고 DOM executor marker, `runtimeTier: local`, `local-strong:` event ID, SQLite append를 확인한다. browser verifier는 실행마다 별도 임시 `CODARO_HOME`을 만들고 종료 뒤 정리하며 PID 재사용으로 이전 evidence가 섞이지 않게 한다. Local strong check는 process 시작·응답 손상·정상 timeout 같은 infrastructure failure만 250ms transport recovery window 뒤 새 sandbox에서 한 번 자동 재시도하고 mismatch와 학생 코드 오류는 재시도하지 않는다. `CODARO_PRODUCT_CASE=local-w0-conformance` 선택군은 Web strong event와 non-credit legacy migration event, Local Day 1, pathlib·zip·schedule 12개 behavior를 순서대로 실행해 Local strong event 16건과 `mixed` archive 18건의 재내보내기·reload를 검증한다. fileOps/zip Local-native event는 sandbox 산출물 descriptor의 path, origin, kind, byteLength, fileCount, SHA-256 contentHash가 payload hash 안에 봉인됐는지도 본다. schedule Local/Web event는 `schedule` 1.2.2 package asset descriptor의 name, version, `check-packages/` URL, SHA-256 integrity가 payload hash 안에 봉인됐는지도 본다. 첫 Web archive의 manifest/event/payload hash, `MigrationImported`의 credit 비부여와 metadata backup, 중복 import, tamper rejection, payload conflict isolation, legacy lessonRef 자동 이관도 검사한다. screenshot과 `product-experience-report.json`을 남기지만, 66개 대표 case와 현재 통과한 18-case Local W0 선택군은 broader CheckSpec coverage, 경합 없는 cold package 준비, Windows AppContainer, document·전체 virtual FS/package set archive, 전체 Chromium/Firefox/WebKit/WebView2·수동 screen-reader release matrix를 대신하지 않는다.
...
- `pyproc-assets-browser`는 editor build 산출물의 `/pyproc-assets.json`을 실제 브라우저 page context에서 읽고, manifest의 5개 entrypoint role과 `vendor/pyproc/**` 파일 전체를 fetch한 뒤 `crypto.subtle.digest`로 SHA-256 SRI를 재계산한다. 이 gate는 `output/test-runner/pyproc-assets-browser/pyproc-assets-report.json`에 `sameOriginVendorUrls`, `sriVerified`, 파일 수와 검증 바이트 수를 남기고, `quality-cycle` summary는 이 report를 `payloadGitHead` evidence로 대조한다.
...
- `learning-content`는 ledger·featured metadata 재현성 검사 뒤 identity, canonical content, path membership, metadata coverage, scored check strength, retrieval/transfer 분리, featured path, featured capstone, 1,400개 solution 실행, assessment authoring report를 모두 현재 `gitHead`로 만든다. registry 472행, category-scoped alias collision 0, canonical owner·outcome·source hash, 31개 path reference, 대표 6경로 49개 explicit metadata, route-backed capstone URL, table·image artifact descriptor, assessed 467레슨의 mastery·transfer·retrieval, weak-only required mission 0을 분리해 기록한다. pinned `packageAssets`는 URL과 SRI를 검사한 wheel만 solution verifier에 주입한다. author review, alias migration review, taxonomy transition review·apply가 빠지면 마지막 completion command가 정확한 blocker와 함께 실패하므로 `planned` 원장이나 machine pass를 완료 증거로 사용할 수 없다.
- `curriculum-top-tier-audit`는 `curriculum-quality-matrix`보다 높은 평가층이다. 작성 절차·lazy uv 의존성·structured source와 함께 strong `CheckSpec`, weak-only 잔존, transfer assessment, retrieval assessment를 필수 영역으로 검사한다. report의 `score`는 `scoreKind: audit-requirement-coverage`인 요구사항 커버리지이며 실제 학습 효과나 제품 품질 점수가 아니므로 `curriculumQualityScore: null`, `completionEligible: false`를 유지한다. 모든 필수 영역을 통과한 경우에만 `topTierEligible: true`가 된다. 결과는 `output/test-runner/curriculum-top-tier-audit/curriculum-top-tier-report.json`에 `summary`, `requirementFailures`, `actionableGaps`와 함께 남긴다.
