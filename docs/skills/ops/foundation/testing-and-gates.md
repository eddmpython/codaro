...
| `plan-quality` | fast | mainPlan이 미완료 TODO만 보존하는지, 현재 사실과 독립 평가 보고서가 완전한지 목표 점수나 통과 점수 없이 검사한다. R10 증거가 green인 뒤에만 blocking·CI required로 승격한다. |
