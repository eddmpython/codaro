...
- `ai-live-smoke`는 opt-in gate다. credential/token이 없으면 skip하지 않고 `live credential missing`을 JSON으로 보고한다. `CODARO_AI_LIVE_PROVIDERS=oauth-chatgpt,openai,ollama,custom`처럼 matrix를 명시하면 provider별 `passed`/`failed`/`credentialMissing` summary를 남긴다. live tool loop는 실제 provider 응답에서 `write-curriculum-yaml`과 `packages-check` 이후 `cell-call`이 발생하는지 본다. 이 gate는 CI required가 아니며, 실제 provider/OAuth/네트워크 문제를 기본 CI 안정성과 분리한다.
