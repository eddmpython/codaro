...
같은 Percent Python source가 SDK, IDE 앱, 전체 publication, 부분 embed와 자동화 proof로 이어지는 제품 여정은 아래 한 명령으로 확인한다.

```powershell
uv run python -X utf8 tests/run.py python-product
```
...
| `python-sdk` | fast | 임시 build context에서 wheel과 sdist를 만들고 작업트리의 package curricula를 바꾸지 않는지 확인한다. 빈 환경의 direct wheel, `uv add --find-links`, exact wheel `uvx`, root public import, server mount, CLI와 package data를 검증한다. |
...
| `markdown-diagram` | surface | Web·Local 공용 Markdown Mermaid 작성 동작, HTML·SVG 정화, 위험 문법 차단, 접근성, Astryx theme·accent 반영, 반응형 폭, viewport 지연 runtime을 실제 Chromium에서 확인한다. |
...
`change-cycle`은 현재 `HEAD` 대비 변경 파일과 untracked 파일을 보고 일반 작업 완료에 필요한 gate만 고른다. 항상 `root-clean`, `docs`를 먼저 실행하고, `src/`·`tests/` 변경은 `backend`, package metadata·root export·distribution builder·packaging verifier·release workflow 변경은 `python-sdk`, proof 실행과 runtime security 경계 변경은 `runtime-security`, `editor/` 변경은 `editor-build`, `gui-control-browser`, `markdown-diagram`, `launcher/` 변경은 `launcher-check`와 `launcher-test`, `landing/` 변경은 `landing-build`, `curricula/` 변경은 `curriculum-quality-matrix`를 추가한다. 커리큘럼 전체 실행성, 브라우저 표면, 제품 품질 판정은 명시 gate나 `quality-cycle`에서 본다.
`python-product`는 `root-clean` → `docs` → `backend` → `architecture-boundary` → `python-sdk` → `app-runtime` → `markdown-diagram` → `publication-compiler` → `static-publication` → `server-publication` → `local-publication` → `block-embedding` → `learning-product-bridge` → `deployment-adapters` → `reference-products` → `automation-ide-audit`의 16개 hard gate를 복사 없이 조합한다. 결과는 `output/test-runner/python-product/sequence-summary.json`에 current `gitHead`, gate별 시간과 log, artifact freshness, `payloadGitHead`, `gitHeadMatches`, `payloadStatus`를 기록한다. 첫 `root-clean`과 각 artifact head가 같은 commit에 결속되지 않으면 제품 완료 증거가 아니다.
`quality-cycle`은 제품이 잘 만들어졌는지 보는 반복 검증 단위다. 앞부분은 `root-clean` → `docs` → `backend` → `runtime-security` 뒤에 `python-product`의 16개 gate 순서를 그대로 포함하고, 이어서 디자인, 접근성, 학습, provider, 설치, 복구, 커리큘럼, 공개 화면, 성능과 launcher gate를 실행한다. 이 명령은 완료 선언을 대신하지 않고 provider, 학습, 자동화, runtime, Astryx 공용 디자인, 대표 실제 화면, Python SDK, publication, 웹 파이썬 자산·파일·서버, 설치/런처, 온보딩, 프론트 성능과 architecture boundary가 한 사이클에서 함께 버티는지 확인한다. 묶음 실행이 끝나면 runner는 통과한 gate 수, soft failure 수, gate별 duration summary, gate별 command log path/size/freshness, 현재 `gitHead`, `startedAt`/`completedAt`, 그리고 gate별 artifact freshness를 `output/test-runner/quality-cycle/sequence-summary.json`에 남긴다. `python-sdk`, `markdown-diagram`, `reference-products`, `product-quality-audit`, `design-system-contract`, `theme-runtime-browser`, `visual-accessibility-browser`, `gui-control-browser`, `run-local-state-browser`, `product-experience-browser`, `astryx-journey`를 포함해 report를 쓰는 gate는 summary 안에 artifact path, fresh 여부, `payloadGitHead`, `gitHeadMatches`, `payloadStatus`가 함께 들어가야 하며 report의 git head가 sequence head와 다르면 artifact failure로 sequence를 실패시킨다. `product-quality-audit`의 10점은 `scoreKind: wiring-coverage`이며 `completionEligible: false`이므로 제품 100점이나 완료 선언으로 쓰지 않는다. 나머지 browser/runtime/curriculum artifact 계약과 live credential soft/hard failure 규칙은 각 개별 gate 설명을 따른다. 이 summary는 제품 SSOT가 아니라 사람이 읽는 완료 증거다.

현재 runner는 `tests/run.py`의 `PYTHON_PRODUCT_GATES`를 same-source 제품 순서의 권위로 사용하고 `PRODUCT_QUALITY_GATES`가 그 ordered subset을 소유한다. publication bundle, 다섯 reference product와 자동화 audit를 생략한 `python-product` 또는 `quality-cycle`은 완료 증거가 아니다.
...
- `python-sdk`는 `docs/skills/ops/tools/buildPythonDistribution.py`가 OS temp 또는 gate scratch의 build context에 current source, `codaro/webBuild`, root `curricula/`를 복사해 wheel과 sdist를 만들게 한다. 실제 작업트리의 `src/codaro/curricula` 존재 여부와 tree hash는 build 전후 같아야 한다. `tests/packaging/verifyPythonSdk.py`는 direct wheel venv, `uv add --find-links` project와 exact wheel `uvx`를 분리하고 `App`, `createServerApp`, `ui`, `codaro.publication`, version, `/codaro/api/health` mount, CLI help, plain Python reference source, editor와 472개 curriculum package data를 확인한다. report는 `output/test-runner/python-sdk/python-sdk-report.json`에 distribution hash와 install별 증거를 남긴다.
...
- `frontend-performance-budget`는 `editor/vite.config.ts`의 Vite 8 `rolldownOptions.output.codeSplitting` 그룹과 build output을 함께 본다. `yaml`, `curriculumSurface`, CodeMirror, Radix, icon, xterm, vendor 경계가 실제 named chunk로 남아야 한다. 현재 검산값은 entry 245,891 bytes, 최대 app-shell CodeMirror 398,272 bytes, app-shell 전체 2,553,114 bytes다. 472개 lazy curriculum은 17,099,958 bytes로 분리하고, viewport에서만 받는 Mermaid runtime은 전용 chunk 1개와 3,500,000-byte 상한으로 따로 추적한다. app-shell의 400,000-byte 단일 JS 예산은 낮추거나 우회하지 않는다. chunk별 크기와 전체 JS/CSS 크기는 `output/test-runner/frontend-performance-budget/performance-report.json`에 남긴다. 이 report의 `gitHead`, `startedAt`, `completedAt`, `durationMs`, `payloadGitHead` evidence는 `quality-cycle` artifact freshness 검증에 들어간다.
