...
현재 runner는 `static-publication`을 `publication-compiler` 직후에 실행한다. 위 순서의 실제 권위는 `tests/run.py`의 `PRODUCT_QUALITY_GATES`이며, 정적 bundle 검증을 생략한 quality-cycle은 완료 증거가 아니다.
