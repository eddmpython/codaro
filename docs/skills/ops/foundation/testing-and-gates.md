...
- `product-browser-webview2-evergreen`은 Windows에서 현재 editor를 build하고 current-commit wheel을 격리 launcher root에 설치한 뒤 실제 `codaro-launcher.exe`의 네이티브 WebView2 창을 연다. CDP는 별도 브라우저를 띄우는 용도가 아니라 해당 WebView2 document를 관찰하는 데만 쓰며, Win32 client 크기와 `innerWidth/innerHeight × devicePixelRatio`를 대조한다. Local Home 900x640, Notebook 1024x768, Automation 1440x900의 runtime tier, 가로 overflow, 상단 테마·공용 SNS, control overlap과 계좌번호 후원 dialog를 검사하고 screenshot과 WebView2 exact version을 `output/test-runner/product-browser-webview2-evergreen/webview2-product-smoke-report.json`에 남긴다. 이 hosted Evergreen 증거는 Windows 10 22H2 self-hosted Fixed Version lock, 수동 보조기술, IME, 200%·400% zoom을 대신하지 않는다.
