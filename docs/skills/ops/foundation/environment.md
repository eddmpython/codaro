...
- Local 세션의 작업 폴더에 같은 이름의 Python 모듈이 있으면 외부 설치 없이 그 파일을 사용한다. 준비 확인은 모듈을 실행하지 않는다. 판정은 `LocalEngine.installPackage`가 소유하며, 에디터는 건너뛴 설치를 새 패키지 설치로 표시하지 않는다.
...
- 설치 timeout은 긴 wheel 준비를 고려해 `INSTALL_TIMEOUT_SECONDS` 상수로 관리한다. timeout 변경은 `tests/runtime/testSystem.py`에서 함께 고정한다.
