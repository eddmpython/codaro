...
- builtins 작성 기준은 `curricula/python/basics/builtins/LOCAL_RUNTIME_COMPATIBILITY.md`에 둔다.
...
- 커리큘럼 폴더에 PRD·설계 메모 같은 기획 문서를 남기지 않는다. 레슨 YAML과 학습 실행에 필요한 자산만 둔다. 설계는 작업 메모로만 하고 폴더에 남기지 않는다.
