...
`comparator: auto`가 일반 출력의 기본이다. line-trim과 대소문자 비교 뒤에도 차이가 있으면 양쪽 전체를 실행하지 않는 제한된 parser로 읽는다. 지원 값은 `None`, bool, 안전 범위의 유한 숫자, 인용 문자열, list, tuple, dict, set이며 중첩도 허용한다. 숫자는 상대 오차 `1e-9`, 절대 오차 `1e-12` 안에서 비교하고, dict와 set 순서는 무시한다. list와 tuple은 서로 다른 타입이며 항목 순서도 유지한다. 한쪽만 해석되거나 제한을 넘으면 값 비교를 적용하지 않는다.

`comparator: text`는 일반 문구에서 영문 대소문자 차이만 허용한다. `comparator: exact`는 `upper()`, `lower()`, 대소문자 구분 검색, 출력 서식처럼 표기 자체가 학습 목표일 때 사용한다. 세 모드 모두 line-trim 밖의 일반 텍스트 공백과 줄 구조를 임의로 바꾸지 않는다. `auto`가 허용한 숫자나 구조 표현 차이는 성공 feedback에 그대로 남는다.
...
- 현재 machine audit의 source 저작 범위는 strong CheckSpec 1,419개/468레슨이며 mastery·transfer·24시간 retrieval은 각각 468레슨이다. 1,402개 assessment solution은 1,400개 behavior와 2개 output 검증으로 실행됐고 실패는 0이다. 이것은 author source 검산이며 제품 strong evidence 지원 범위와 다르다. Web behavior는 `localRequired`로 strong event 0을 유지한다. 일반 개발 서버의 Local behavior는 practice로 남지만, 설치형 WebView2에서는 AppContainer 응답과 지원 Windows build를 확인한 뒤 Local strong event를 실제 SQLite에 저장한다. Fixed Version release gate가 이 UI 저장 경로와 broker 경계, 공유 ACL 회수, cold package 경합을 한 설치본에서 검사한다. Web Day 1 strong event와 legacy migration event는 Local import·재내보내기·Web reload 뒤에도 원래 runtime identity를 유지한다. 사람 효능 근거는 전체 scheduler 구현 완료가 아니라 경로별 공개 승격 상태를 결정한다.
