...
- 구체적 학습 요청이 실제 provider 응답에서 `packages-check → write-curriculum-yaml` 필수 prefix로 이어지고, provider가 보낸 YAML이 실제 materializer에서 섹션/스니펫/실습 셀과 `contractGapCount=0`으로 변환되는지.
...
- concrete learning request는 답변만으로 끝나지 않고 `packages-check → write-curriculum-yaml` 필수 prefix를 만족하며, 그 YAML은 structured section card 계약을 통과해야 한다. 이후 `read-cells` 같은 읽기 전용 검증은 허용하되 불필요한 쓰기/실행 tool은 허용하지 않는다.
