...
`tests/teacher/testAiProvider.py`는 callback 성공 후 같은 profile/secret store로 teacher 질문을 준비해 `oauth-chatgpt` provider config가 생성되고 응답 payload가 반환되는 경로를 검증한다. live credential을 쓰는 외부 smoke와 별개로, OAuth login → callback → token 저장 → profile active provider 갱신 → 질문 준비/응답 경로의 제품 내부 연결은 이 테스트가 담당한다.
...
- 구체적 학습 요청이 실제 provider 응답에서 `resolve-learning-goal → search-curricula → compose-master-plan` 추천·조합을 먼저 거치고, compose 결과가 gap을 보고할 때만 `packages-check → write-curriculum-yaml`로 이어지는지. provider가 보낸 YAML은 실제 materializer에서 섹션/스니펫/실습 셀과 `contractGapCount=0`으로 변환되어야 한다.
- YAML 작성 뒤 결과 확인을 위한 `read-cells` 같은 읽기 전용 inspection tool은 허용하지만, 쓰기/실행 tool이 끼면 실패로 본다.
...
- concrete learning request는 답변만으로 끝나지 않고 `resolve-learning-goal → search-curricula → compose-master-plan` 추천·조합을 먼저 만족해야 한다. compose 결과가 gap을 보고할 때만 `packages-check → write-curriculum-yaml`이 이어지며, 그 YAML은 structured section card 계약을 통과해야 한다. 이후 `read-cells` 같은 읽기 전용 검증은 허용하되 불필요한 쓰기/실행 tool은 허용하지 않는다.
...
- tool 결과를 돌려준 뒤 다음 provider 호출이 `provider_network_error`로 끊긴 경우는 prompt/schema tuning 실패로 오분류하지 않는다. live smoke는 recoverable network case를 제한적으로 재시도하고, 통과하더라도 `attemptCount`, `networkRetryCount`, `previousNetworkFailures`, `recoveredAfterNetworkRetry`를 report에 남긴다.
...
- `tests/surface/testProviderSettingsContract.py`
