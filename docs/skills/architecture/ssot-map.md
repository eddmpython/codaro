...
| Markdown 다이어그램 | `docs/skills/architecture/markdown-diagrams.md`, `editor/src/lib/markdownPreview.ts`, `editor/src/lib/mermaidDiagram.ts` | Web·Local 공용 fenced Mermaid 원문, typed segment 보안 경계, Astryx 토큰, 접근성, viewport 지연 로딩 기준 |
...
| 브랜드 마크 치수 | `assets/brand/designSystem/brandMark.json`, `landing/scripts/syncBrand.js`, `editor/scripts/syncBrandMark.mjs` | wordmark와 chrome/footer/hero 아바타 px·gap, favicon 원본 px의 SSOT. 두 프론트엔드가 각자 자기 lifecycle에서 `src/lib/generated/brandMark.json` 사본을 만든다(추적하지 않음) |
...
| 기준 | 파일 | 역할 |
|---|---|---|
| retired assignment room architecture | `docs/skills/architecture/assignment-room.md` | 제거된 과제방 경계와 local-owner 데이터 이관 계약 |
...
| classroom migration regression | `tests/migrations/testClassroomArchive.py` | count/hash, redaction, exact-hash purge, lock과 crash recovery 회귀, 실제 app route table의 classroom 경로 0건 |
| classroom removal regression | `tests/architecture/verifyClassroomRemoved.py`, `tests/learning/verifyRemovedLearningConcepts.py` | active backend/frontend/source, `/api/classroom` HTTP surface와 학습 side effect 재유입 차단 |
