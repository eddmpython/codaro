...
| exercise check dispatch | `src/codaro/curriculum/exerciseCheck.py` | check type별 실행 분기와 check response payload 기준 |
...
| turn runtime | `src/codaro/ai/teacher/turnRuntime.py` | context 주입, turn session, tool executor를 provider 실행 단위로 조립 |
...
| completion runtime | `src/codaro/ai/completion.py` | editor code completion prompt, context 축약, provider 호출 기준 |
...
| workloop state | `editor/src/lib/workLoop.ts` | tool_start/tool_results와 response trace summary를 UI state로 변환 |
