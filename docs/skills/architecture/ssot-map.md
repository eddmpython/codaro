...
| provider loop | `src/codaro/ai/teacher/providerLoop.py` | provider tool call을 conversation, policy, executor, trace에 반영 |
| provider stream | `src/codaro/ai/teacher/providerStream.py` | streaming turn event, token stream, tool lifecycle event를 생성 |
...
| manifest | `src/codaro/ai/toolManifest.py` | group, lane, category, target, risk 메타데이터 |
...
| curriculum selection | `editor/src/lib/curriculumSelection.ts` | 기본 커리큘럼 fallback, 콘텐츠 선택, 나만의 커리큘럼 선택 상태 |
