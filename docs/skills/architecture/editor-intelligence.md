...
도구의 배치는 [셀 강행규칙](../ops/product/branding.md#셀의-간결함은-변경-불가한-강행규칙)을 따른다.
`CodeIntelligenceTools`는 기존 더보기 메뉴나 단축키로 요청한 작업을 별도 대화상자에서 열고,
하나로 확정된 정의는 편집 위치로 바로 이동한다. 작업 목록과 단축키 표시는 같은 action 목록이
소유한다. 셀 편집기의 렌더링 영역에는 도구 대화상자가 포함되지 않는다.
...
달라지면 늦게 도착한 분석 결과나 변경을 적용하지 않고 이전 버전의 진단·탐색 결과도 지운다. 분석 파일은 임시 작업 공간 안에만
...
`InlineRepair`는 수정 요청 대화상자에서 원본과 제안을 함께 보여주며 `repairChanges.ts`가 선택한 변경만 적용한다. 요청 중
...
상태 복원은 해당 코드 버전의 실행 이력을 선택한 경우에만 허용한다. Python 파일과 열린 파일의
위치는 런타임 체크포인트가 함께 보존하며 복제 프로세스의 파일 수정은 원본에 반영하지 않는다.
...
주소가 고정된 실행 자산과 자산 목록은 네트워크에서 재검증하고 오프라인일 때 같은 범위의 저장본을
사용한다. 파일 이름에 해시가 포함된 앱 번들만 캐시를 먼저 읽어 이전 런타임이 새 배포에 남지 않게 한다.
...
빈 셀과 작성한 셀에 보조 도구 행이 없는지, 메뉴와 대화상자를 열고 닫아도 셀 크기가 보존되는지도
같은 검수 경로에서 확인한다. `--mobile`은 좁은 화면에서 같은 편집 흐름을 검사한다.
...
대체하지 않는다. 같은 서버에 `--live-provider`를 주고 검수 스크립트를 `--repair-live`로 실행하면
현재 copilot provider의 실제 응답으로 동일한 편집 흐름을 검사한다.
`--execution-control`은 실행 중지까지, 기본 실행은 파일 상태 복제·복원까지
