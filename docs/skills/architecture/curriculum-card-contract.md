...
1. **화면 장식에 이모지를 쓰지 않는다.** `emoji`, `titleEmoji`, `endEmoji`는 레거시 입력으로만 허용하고 렌더러는 표시하지 않는다.
2. **아이콘은 카드 의미에서 자동 매핑한다.** 저자는 `displayKind`, `role`, `sourceType`을 정확히 고르고, 화면은 Lucide 아이콘으로 표현한다.
3. **카드 내부 색은 최소화한다.** 기본은 `bg-card`/`bg-background`, `border-border`, `text-foreground`, `text-muted-foreground`다.
4. **색은 상태 표식일 때만 쓴다.** 위험·경고·성공처럼 의미가 있는 경우 얇은 rail, 아이콘 tint, 작은 상태 배지 하나로 제한한다.
5. **순번 기반 무지개 카드 금지.** 같은 그리드의 1번/2번/3번 카드가 서로 다른 색이 되는 방식은 학습 시선을 흐리므로 쓰지 않는다.
6. **설명카드는 학습 행동으로 이어져야 한다.** 소개형 카드 나열 뒤 실행·검증 없이 다음 개념으로 넘어가면 실패한 카드다.
