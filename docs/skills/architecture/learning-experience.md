...
이 원칙은 공개 레슨과 편집기 모두의 `직접 해보기` 영역에 적용한다. 해당 영역은 실습 지시문만
제공하며 시작 코드를 읽기 전용 코드 블록으로 보여주는 것도 금지한다. 학습자가 직접 타이핑한다.
...
학습 목차는 왼쪽 탐색 사이드바와 분리해 화면 오른쪽 끝에 일정한 폭으로 배치한다.
마우스나 키보드 초점으로 펼쳐져 본문을 밀어내지 않는다. 본문 스크롤에 따라 현재 섹션을
표시하고 목차 링크로 해당 섹션에 이동한다. 좁은 화면에서는 본문을 우선하고 기존 레슨 개요의
섹션 이동을 사용한다. 코드 입력의 들여쓰기는 공용 편집기의 `codeIndentation.ts`를 따른다.
