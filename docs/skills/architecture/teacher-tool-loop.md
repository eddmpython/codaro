...
1. **요청 분류**
   ...
   - clarification gate가 멈춘 대화는 `pendingClarification`으로 `assumptions` payload를 대화 상태에 저장한다. 다음 턴에서 사용자가 `진행`하거나 짧게 답하면 이 payload를 `[Clarification plan]`으로 provider prompt에 주입하고 한 번 소비한다. 이전 assistant 텍스트만 다시 읽게 만들면 실패다.
...
7. **Trace와 평가**
   ...
   - clarification continuation golden case는 첫 turn의 `pendingClarification.assumptions`가 다음 provider turn의 `[Clarification plan]`에 machine-readable JSON으로 들어가고, `defaults` alias 없이 소비되는지 확인한다.
