...
## Diagnosis Lane - 실행, 피드백, 교정, 전이

학습자가 코드를 실행하면 strong check 결과가 [[learnerState]] store를 갱신한다. 별도 예측 입력이나 확인 동작을 요구하지 않는다. teacher orchestrator는 실제 통과 여부, 체크 강도, 코드와 에러에서 다음 행동을 정한다.

기본 진단 흐름:
1. **실행** - 학습자가 셀을 실행하고 Web 또는 Local runtime이 결과를 수집한다.
2. **피드백** - strong check가 출력, 변수, 동작, 생성 artifact를 즉시 검증한다.
3. **교정** - 실패한 코드와 에러만 `match-misconception`에 전달해 관찰 가능한 오개념을 매칭한다.
4. **전이** - `suggest-next-step`이 mastery 하한, 강한 관측 수, 미해결 misconception으로 교정, 반복, 다음 outcome을 결정한다.
