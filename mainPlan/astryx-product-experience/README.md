...
| 영역 | 현재 구현 | 현재 판정 |
| --- | --- | --- |
| Astryx 공용 기반 | Landing과 editor가 exact `@astryxdesign/* 0.1.6`, StyleX `0.19.0`, 공용 token/provenance와 cascade layer를 소비한다. | `design-system-contract`는 current `gitHead`, 시작·완료 시각, duration, report path를 포함한 artifact 계약까지 green. 실제 모든 engine·AT matrix 완료는 아님 |
