...
- Local-native Python provisional sandbox는 Day 1과 W0 filesystem·zip·schedule base·assessment solution을 판정한다. launcher AppContainer broker source와 현재 Windows 11 직접 OS 경계 test는 있지만 목표 Windows 10 설치본 conformance와 shared runtime ACL receipt·GC가 없어 `practice` 피드백만 제공하고 Local strong event는 0건이다. Web behavior도 `localRequired`로 strong event 0을 유지한다. Web strong·legacy migration 2건의 archive를 Local이 가져오고 재내보내도 Web runtime identity와 exact event set이 보존된다.
