...
| 02 | [landing-and-learning-migration](_done/02-landing-and-learning-migration/) | 진행 | 웹 우선 홈과 읽히는 학습 카탈로그, 실제 제품 이미지, direct lesson |
...
00의 token·package contract와 01의 theme runtime contract는 각각 clean implementation commit, evidence commit과 completion transition을 마쳤다. 실제 전체 시각·접근성 검토는 04가 소유하므로 사람 시각 검토를 00·01에 중복 선행 조건으로 걸지 않는다. 02~04는 계속 active이며 workstream 전체 증거는 아직 작업 중이다.
