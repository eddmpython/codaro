...
현재 source Pages workflow는 `npm ci` 뒤 editor를 `/codaro/run/`과 `/codaro/app/` 두 base로 각각 build하고 Landing을 조립한다. `/app/` tree는 아직 C0 immutable archive나 C2 redirect가 아니라 current editor의 두 번째 build다. `main@088a5df1` GitHub Pages에서 `/run/`과 canonical Day 1 route의 실제 edit·run·strong check·archive export는 통과했지만, C0 archive 고정과 두 release compatibility handoff가 없으므로 C1/C2 전체를 통과로 쓰지 않는다.
