...
현재 source Pages workflow는 current editor를 `/codaro/run/`으로만 build한다. `/codaro/app/`은 `contracts/webCompatibilityC0.json`의 source commit을 LF로 별도 checkout해 exact tree hash를 확인한 artifact만 복원한다. 2026-07-27 captured Pages artifact와 공개 URL crawl은 632개 전체 파일·23,761,225 byte·response type이 일치했고, fresh checkout 조립 검증도 current `/run/`과 pinned `/app/`을 분리한다. Windows `core.autocrlf=true`가 curriculum YAML을 CRLF로 바꿔 bundle을 약 716KB 증가시키던 재현성 문제는 `.gitattributes`와 pinned checkout의 `core.autocrlf=false`로 차단했다. 다만 정식 C0 release asset URL·archive SHA-256과 두 release compatibility handoff가 없으므로 C0/C1/C2 전체를 통과로 쓰지 않는다.
