...
남은 종료 조건은 self-hosted Windows 10 22H2에서 Fixed Version release gate를 실제 통과한 current-commit 근거, 수동 screen-reader matrix, 대표 학습자 사용성 근거, 정식 `/app/` C1~C3 두 release·tombstone·28일 telemetry 근거, 독립 R10 raw report와 archive·browser 경계 독립 보안 검수다. exact Fixed Version lock·설치 무결성·200% browser zoom·400% text-only fixture의 machine 경계는 구현됐다. identity·content 472/472, taxonomy 전이와 independent assessment 468/468 승인은 `contracts/learning-content/`에서 완료됐다.

## 목표

Astryx migration을 보기 좋은 screenshot 몇 장으로 끝내지 않는다. Landing, Learn, Run, Local과 대표 교육 경로가 접근성, 반응형, 성능, 상태 완전성, 실제 배포에서 통과해야 이 TODO와 이니셔티브를 삭제한다.
...
| `product-browser-webview2-evergreen` | launcher/frontend | yes | Windows every change | current wheel과 실제 native WebView2의 Local 3개 viewport, 공용 control, Web-origin 학습 archive 왕복 |
| `product-browser-webview2-win10` | launcher/frontend | no | release self-hosted | Windows 10 22H2 build 19045, exact Fixed Version runtime, Local 3개 surface의 200% browser zoom·400% text-only fixture와 native product matrix |
...
| `visual-accessibility-browser` | Landing·Editor production build, locked Playwright의 Chromium·Firefox·WebKit 12-case와 report unit test |
...
| `product-browser-webview2-win10` | lock schema·30일 freshness·official URL·archive/executable SHA-256 검사, Fixed Version 설치·AppContainer ACL, native launcher runtime identity와 zoom matrix |
...
`product-browser-webview2-evergreen`은 구현됐고 `.github/workflows/ci.yml`의 hosted `windows-latest` job이 current stable native smoke를 실행한다. `.github/workflows/pages.yml`은 실제 Pages deploy 성공 뒤 `page_url`을 Windows 후속 job의 `CODARO_DEPLOYED_WEB_URL`로 넘겨 공개 Web edit·strong check·archive export와 설치형 Local import·reload·re-export 경로를 실행하고 report·screenshots·다운로드 파일을 artifact로 남긴다. 공용 verifier는 editor build, current-commit wheel 조립·설치, gate 전용 `CODARO_HOME`, 실제 launcher EXE, WebView2 CDP 관찰, Win32 client resize, 3개 Local surface와 공용 control, 두 종류 Web-origin 학습 archive 왕복을 한 report로 묶는다. 재내보내기는 evidence manifest 재생성과 Local evidence set union으로 root hash가 달라질 수 있어 document·drafts·virtual FS·packages·automation drafts와 source evidence events를 materialize한 의미 동일성으로 판정한다.

release blocker `product-browser-webview2-win10`은 별도 runtime profile로 구현됐다. `tests/product/webview2-runtime.lock.json`은 exact version 151.0.4129.59, release date, Microsoft 공식 archive URL, 304,114,944 bytes, archive·`msedgewebview2.exe` SHA-256, 고정 설치 경로와 30일 freshness를 잠근다. 설치 도구는 CAB를 resume·hash 검증 뒤 `expand.exe`로 풀고 Windows 10 Fixed Version 120+에 필요한 두 AppContainer SID의 read/execute ACL을 부여한다. gate wrapper는 Windows 10 22H2 build 19045와 interactive desktop을 강제하고 `WEBVIEW2_BROWSER_EXECUTABLE_FOLDER`를 lock 경로로 설정하며 CDP의 exact `Edg/<version>`과 실행 파일 hash를 다시 대조한다. 공용 native verifier는 실제 900x640 client에서 Home·Notebook·Automation 각각의 200% browser zoom과 계산된 요소별 글자 크기만 4배로 바꾸는 400% text-only fixture를 실행한다. 여섯 snapshot은 horizontal overflow, viewport 밖 control, top-control overlap, 내부 text clipping, visible text overlap이 모두 0이어야 한다. fixture는 각 snapshot 직후 원래 inline style을 복구해 후속 Automation·IME 상태를 오염시키지 않는다. `.github/workflows/release-quality.yml`의 product release job은 정확히 `[self-hosted, windows, x64, codaro-win10-22h2, interactive]` labels에서만 이 blocker를 호출한다. 현재 저장소에는 해당 self-hosted run의 current-commit 통과 report가 없으므로 10 완료나 Windows 10 conformance를 주장하지 않는다. minimum supported runtime compatibility는 release-candidate lock과 분리하고 오래된 runtime을 release 기본 설치물로 되살리지 않는다. Evergreen green도 이 self-hosted 근거를 대신하지 않는다.

표의 `product-release`는 개별 `Gate`가 아니라 `PRODUCT_RELEASE_GATES` tuple, `product-release` CLI subparser, `GATE_ARTIFACTS["product-release"]`를 가진 sequence다. 기존 quick `preflight`를 재정의하지 않는다. 현재 순서는 `root-clean`, `docs`, `backend`, `architecture-boundary`, `editor-build`, `landing-build`, `mobile-layout`, `frontend-performance-budget`, `design-system-contract`, `theme-runtime-browser`, `visual-accessibility-browser`, `learning-method`, `curriculum-quality-matrix`, `path-promotion-readiness`, `repository-simplification`, `learning-content`, `web-learning`, `visual-assets`, `landing-public`, `removed-learning-concepts`, `product-experience-browser`, `astryx-journey`, `local-studio-browser`, `run-local-state-browser`, `learning-evidence-contract`, `learning-efficacy-report`, `automation-ide-audit`, `launcher-test`, `product-browser-webview2-win10`, `path-learning-signal`, `evaluation-contract`, `plan-quality`, `r10-independent-review`다. self-hosted Windows 10 Fixed Version gate와 non-required인 사람·독립 증거 gate도 release aggregate에서는 실행하고 하위 실패를 aggregate success로 바꾸지 않는다. `path-efficacy-confirmatory`는 featured path 승격 조건이며 shell artifact 배포 sequence와 분리한다.
...
- 한 화면만 예쁜 상태, 웹에서 학습을 끝낼 수 없는 상태, 완료 근거가 없는 상태는 출시 또는 TODO 삭제 조건을 충족하지 않는다.
