...
### 방향 교정 + 듀얼 런타임 노트북 (2026-07-12, 사용자 지시)

- **교정 1 - 중복 노트북 금지** (`ca260464`): 랜딩에 새로 만든 학습 노트북(lessonPage·pythonRunner·레슨 JSON)을 전부 걷어냄. 웹 학습은 별도 UI 신설이 아니라 **기존 editor를 브라우저에서 그대로 돌리는 것**. /learn은 카탈로그로만.
- **교정 2 - astryx 정본 준수** (`2277727a`): 손 CSS로 뭉갠 랜딩을 astryx CLI 템플릿(`npx astryx template centered-hero`) 패턴으로 전면 재작성. VStack/HStack(gap+hAlign)·Heading(textWrap=balance)·Grid(minWidth)·Card/Badge 조합 + 중앙 셸/풀블리드 밴드. Text type/color 유효값(body/label, primary/secondary)만. **헤드리스 Chrome 스크린샷으로 반복 검증**(블라인드 디자인 금지).
- **듀얼 런타임 노트북** (`2d3081ff`): notebookRuntime의 오프라인 경로가 가짜 실행(localRuntime print 정규식) 대신 **pyproc 브라우저 커널로 진짜 실행**. browserPythonRuntime.ts = 커널(싱글턴 boot, stdout/stderr 캡처, 마지막 표현식 repr, 변수 추출 typeName/repr/size/shape/dtype, 상태 델타, 패키지 보장 배포판→micropip). 노트북 전체 실행은 문서 순서 순차(리액티브 그래프는 백엔드 소유). 같은 노트북이 apiOnline이면 로컬 백엔드, 아니면 브라우저 WASM CPython.
- **에디터 웹 서빙** (`1864a535`): editor vite에 CODARO_WEB_BASE/CODARO_WEB_OUT env 변형(base는 슬래시 없이 받아 정규화 - Git Bash MSYS 경로 변환이 선행 "/"를 윈도 경로로 망가뜨림). pages.yml이 에디터 웹 변형을 landing/static/app로 빌드 → **https://eddmpython.github.io/codaro/app/ 라이브 200 확인**. 랜딩 홈·학습에서 "브라우저에서 노트북 열기" 연결. 헤드리스 캡처로 에디터 부팅(사이드바·채팅 홈) 확인.
