...
- **P0(착수)**: 설계 문서(이 문서) + astryx 툴체인 실증(landing에 설치, 최소 astryx 페이지 빌드 통과). ← 이번 사이클.
- **P1**: 브랜딩 토큰 + 랜딩 홈 astryx 재구현, Pages 유지 빌드 green.
- **P2**: `/learn` 학습 페이지(정적 커리큘럼 + astryx 표면), 실행 없이 콘텐츠 먼저.
- **P3**: pyproc PyodideEngine 배선 - 학습 레슨 셀 실행을 브라우저에서(순수 파이썬부터).
- **P4**: SW + AsgiServer로 codaro FastAPI 브라우저 서빙(browser-as-server), `/api/*` 라우팅.
- **P5**: WS 어댑터(kernel), 영속(IndexedDB), capability router 배지, COOP/COEP 호스팅 이전.
