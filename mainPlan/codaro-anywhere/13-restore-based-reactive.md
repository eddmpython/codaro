...
**돌파 (2026-07-11, 토론->실측->수정 루프로 뚫음): 단일 수정(완전 해시)으로 성장 워크로드까지 sound**

초기엔 메모리 키우는 셀(pandas) 후 복원이 `memory access out of bounds` 크래시.
- **진짜 원인 = 불완전 델타**: page 해시를 32바이트 샘플링해 변경을 놓침 -> base+delta 복원이 어긋난 힙 생성 -> 크래시. **수정 = Uint32 워드 단위 완전 해시**(모든 바이트 커버). 델타가 완전해져 복원이 이전 상태를 비트단위 정확 재구성.
- **정정(자기교정)**: 처음엔 "메모리 성장(WASM grow만/shrink불가)"도 원인이라 보고 미리성장(천장) 수정을 같이 넣었으나, **실측 재확인 결과 미리성장은 불필요했다(red herring)**. 완전 해시만으로 성장도 처리됨: 미리성장 없이 힙이 43->89MB로 커진 뒤에도 복원이 sound(marker 되돌림, df 사라짐, 인터프리터 생존). 이유 = 복원이 dlmalloc break 포인터(선형메모리 안 바이트)를 옛 값으로 되돌리면 malloc이 [oldBreak, 현재크기) 구간을 memory.grow 없이 재흡수(append-only 특성). **즉 "동적 천장"은 별도 구현 없이 완전해시로 이미 해결.**
...
**동적 천장 = 해결됨 (실측 PASS, 별도 구현 불필요)**: 완전 해시가 성장을 자동 처리. 미리성장 없이 힙이 43->89MB로 커진 전체 리액티브 시나리오에서 라이브복원 3.2ms(13.56MB), 편집 결과 전체재실행과 정확 일치, 크래시 0. 천장을 추측/재성장할 필요 자체가 없음(sbrk break를 복원이 되돌리면 malloc 재흡수).
