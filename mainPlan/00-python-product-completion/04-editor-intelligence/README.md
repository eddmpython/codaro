...
자산 복구 후 `a39cf214`에서 전체 preflight를 다시 실행해 root-clean·docs·backend 3개 gate가
통과했다. Python 테스트는 2004개 통과, 8개 건너뜀, 하위 검사 124개 통과였고 실패는 없었다.
이 결과는 아래의 실제 모델 인증 및 pyproc 파일 상태 계약 실패를 해소한 근거가 아니다.
