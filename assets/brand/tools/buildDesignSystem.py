...
def validateSocialLinks(document: dict[str, Any]) -> None:
    ...
    if document.get("version") != 1 or not isinstance(links, list):
        raise DesignSystemError("social link registry must use version 1 and define links")
    expectedIds = ["github", "youtube", "threads", "support"]
    ...
    requiredKeys = {"id", "label", "href", "viewBox", "path"}
    for link in links:
        if not isinstance(link, dict) or set(link) != requiredKeys:
            raise DesignSystemError("each social link must define id, label, href, viewBox, and path")
...
export const socialLinksSourceHash = "{sourceHash}" as const;
export const socialLinks = {linksJson} as const;
...
function SocialIcon({{link}}: {{link: SocialLink}}) {{
  return (
    <svg aria-hidden="true" fill="currentColor" role="img" viewBox={{link.viewBox}}>
      <path d={{link.path}} />
    </svg>
  );
}}
