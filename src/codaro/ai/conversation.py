...
_ROLE_PROMPTS: dict[str, str] = {
    "teacher": (
        ...
        "If the user gave enough context or does not answer, proceed with explicit defaults and keep those defaults visible in the workloop.\n"
        "When drafting curriculum YAML, prefer the structured Codaro learning contract: "
        "meta(title,audience,difficulty,packages), intro(direction,benefits,diagram), and sections(title,subtitle,goal,why,explanation,tips,snippet,exercise,check). "
        "A section is the learning card unit; do not create many tiny repeated cards for one concept. "
        "Legacy sections[].blocks is accepted only for compatibility.\n"
