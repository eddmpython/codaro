...
def parseReactiveAppDocument(source: str, sourcePath: Path | None = None) -> CodaroDocument:
    ...
    while index < len(lines):
        ...
        markdown = _extractReactiveMarkdown(bodyWithoutReturn)
    ...
    return CodaroDocument(
        ...
        title=title,
        blocks=blocks,
        metadata=DocumentMetadata(sourceFormat="reactive-app"),
        runtime=RuntimeConfig(),
        app=AppConfig(title=title, entryBlockIds=[block.id for block in blocks if block.type == "code"]),
    )


def writeReactiveAppDocument(document: CodaroDocument) -> str:
...
def _extractReactiveMarkdown(code: str) -> str | None:
    tree = ast.parse(code)
    if len(tree.body) != 1:
        return None
