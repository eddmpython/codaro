...
- `document/`
  - 문서 모델
  - codaro/percent/ipynb 파서와 writer
  - 문서 서비스
- `kernel/`
  - Python 실행 세션
  - 세션 관리자
...
4. 내보내기는 `/api/document/export`에서 Codaro/percent/Jupyter writer를 사용한다
...
4. Python 문서는 `codaro`, `jupyter` 타입으로 분류해서 프론트 홈에 전달한다
...
- 프론트 시각 원칙과 UI 톤의 source of truth는 `editor/DEV.md`에 둔다
  ...
  - 공용 chrome은 `shadcn` 패턴, 편집기 셀 리듬 기준도 그 문서가 source of truth다
