...
SOURCE_ROOT = REPO_ROOT / "curricula" / "python" / "basics" / "30days"
...
def makeIntroMarkdown(dayEntry: dict[str, object], content: dict[str, object]) -> str:
    ...
    return cleanMarkdown(
        ...
        이 노트북은 `curricula/python/basics/30days/{dayEntry["file"]}` YAML을 원본으로 생성했습니다. 위에서 아래로 읽고 실행하되, 연습 셀은 일부러 비워둔 공간입니다.
...
def writeManifest(dayEntries: list[dict[str, object]]) -> None:
    ...
    manifest = {
        "title": "Python 30일 완성",
        "source": "curricula/python/basics/30days/curriculum.json",
        "description": "curricula/python/basics/30days YAML을 원본으로 생성한 Colab 학습 노트북",
        "days": [
            {
                "day": int(entry["day"]),
                "title": entry["title"],
                "sourceYaml": f"curricula/python/basics/30days/{entry['file']}",
