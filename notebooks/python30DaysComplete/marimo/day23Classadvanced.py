...
def _(mo):
    ...
    오늘은 `상속`, `super`, `오버라이드`, `다형성`, `상속보다 합성 판단`만 집중합니다. 한 번에 너무 많이 배우면 어디서 막혔는지 찾기 어렵기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 내용은 `클래스 고급`입니다. 예제 셀을 실행하기 전에 아래 순서로 천천히 따라가 봅니다.
...
def _(mo):
    ...
    `PremiumUser`가 `User`를 상속하고 `canExport`가 `True`가 되도록 실행해 확인하세요.
...
def _(mo):
    ...
    2. **값 바꾸기**: 비슷한 문제 `부모 클래스의 초기화를 재사용해 관리자 사용자를 만드세요.`에서 숫자나 문자열을 하나 바꾸고 확인 코드도 함께 고칩니다.
