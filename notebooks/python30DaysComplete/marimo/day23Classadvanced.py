...
def _(mo):
    ...
    여러 클래스가 비슷한 속성과 동작을 공유할 때 매번 복사하면 유지보수가 어려워진다. 상속은 공통 부분을 부모로 올리고 차이만 자식에서 표현하게 해준다.
    
    ## 생각 모델
    
    상속은 '종류' 관계를 표현할 때만 쓰는 것이 좋다. 자식은 부모처럼 행동할 수 있어야 하고, 필요한 부분만 더하거나 바꾼다.
    
    ## 자주 하는 실수
    
    - 공통 코드 복사로 상속 효과를 없애기
    - `super().__init__` 호출을 빼먹기
    - 상속이 맞지 않는 관계에 억지로 쓰기
    """)
    return
...
def _(mo):
    ...
    1. `상속`를 한 문장으로 설명할 수 있는가?
    2. `super`를 잘못 쓰면 어떤 결과나 에러가 날 수 있는가?
...
def _(mo):
    ...
    오늘의 핵심 범위는 `상속`, `super`, `오버라이드`, `다형성`, `상속보다 합성 판단`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `클래스 고급`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    | 줄 | 코드 | 역할 |
    |---:|---|---|
    | 1 | `class Animal:` | 데이터와 동작을 묶을 새 타입의 설계도를 만듭니다. |
    | 2 | `    def speak(self):` | 재사용할 동작에 이름을 붙입니다. 입력과 반환값을 함께 생각합니다. |
    | 3 | `        return "sound"` | 함수 호출자에게 돌려줄 결과를 정합니다. |
    | 4 | ` ` | 읽기 좋게 구획을 나누는 빈 줄입니다. |
    | 5 | `class Dog(Animal):` | 데이터와 동작을 묶을 새 타입의 설계도를 만듭니다. |
    | 6 | `    def speak(self):` | 재사용할 동작에 이름을 붙입니다. 입력과 반환값을 함께 생각합니다. |
    | 7 | `        return "bark"` | 함수 호출자에게 돌려줄 결과를 정합니다. |
    | 8 | ` ` | 읽기 좋게 구획을 나누는 빈 줄입니다. |
    | 9 | `pet = Dog()` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 10 | `pet.speak()` | 마지막 표현식이거나 호출입니다. 실행 결과를 관찰해 상태를 확인합니다. |
    """)
    return
...
def _(mo):
    ...
    자식 클래스에 같은 이름의 메서드가 있으면 부모 메서드와 어떤 관계가 될까요?
...
def _(mo):
    ...
    `PremiumUser`가 `User`를 상속하고 `canExport`가 `True`가 되도록 완성하세요.
...
def _(mo):
    ...
    아래 자식 클래스는 부모 초기화를 호출하지 않아 `name` 속성이 없습니다. `super()`를 추가하세요.
...
def _(mo):
    ...
    부모 클래스의 초기화를 재사용해 관리자 사용자를 만드세요.
...
def _(mo):
    ...
    **랩 목표**: 알림 기본 클래스와 이메일/문자 알림 클래스를 만들고 같은 `send` 메서드 이름으로 다르게 동작하게 하세요.
...
def _(mo):
    ...
    알림 기본 클래스와 이메일/문자 알림 클래스를 만들고 같은 `send` 메서드 이름으로 다르게 동작하게 하세요.
...
def _(mo):
    ...
    **변형 과제**: 새로운 `PushNotification` 클래스를 추가해 같은 리스트에서 동작하게 해보세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `부모 클래스의 초기화를 재사용해 관리자 사용자를 만드세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
    3. **역문제**: 결과값을 먼저 정하고, 그 결과가 나오도록 입력 데이터를 설계합니다.
    4. **오류 만들기**: 오늘의 자주 하는 실수 중 하나를 일부러 만들고, 에러 이름이나 잘못된 결과를 기록합니다.
    5. **설명하기**: 상속, super, 오버라이드 중 하나를 비전공자에게 설명하는 3문장 메모를 씁니다.
...
def _(mo):
    ...
    - 오늘 내가 배운 핵심은 `클래스 고급`이고, 한 문장으로 말하면:
    - 내가 고친 버그의 원인은:
