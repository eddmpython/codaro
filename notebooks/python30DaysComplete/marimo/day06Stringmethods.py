...
def _(mo):
    ...
    이 노트북은 `curricula/python/30days/day06_문자열메서드.yaml` YAML을 원본으로 생성했습니다. 위에서 아래로 읽고 실행하되, 연습 셀은 일부러 비워둔 공간입니다.

    ## 오늘의 목표

    문자열 메서드로 대소문자, 분리, 결합, 치환, 공백 제거, 검색을 처리합니다.

    ## 왜 중요한가

    현실의 텍스트 데이터는 공백, 대소문자, 구분자가 섞여 있어 정리 과정이 필요합니다.

    ## 생각 모델

    메서드는 값 뒤에 붙여 쓰는 동작 버튼입니다. 원본 문자열을 읽고 새 결과를 만들어 냅니다.
...
def _(mo):
    ...
    - 오늘은 일부러 쓰지 않는 개념: 리스트 메서드, 직접 만드는 함수, import
...
def _(mo):
    ...
    - 코딩 전: 점 앞의 값이 무엇이고 점 뒤의 동작이 무엇인지 나누어 읽습니다.
    - 실행 중: 메서드 실행 후 원본 변수와 결과 값을 따로 확인합니다.
    - 연습 초점: 문장을 단어로 나누고 다시 합치며 split()과 join()의 짝을 익힙니다.
    - 막히면: AttributeError가 나면 점 앞의 값이 문자열인지 확인합니다.
    """)
    return
...
def _(mo):
    ...
    - split()으로 문자열을 나누고 join()으로 다시 합칠 수 있다.
    - replace(), strip(), find(), count()의 쓰임을 예로 설명할 수 있다.
    - 문자열 메서드가 새 값을 만든다는 점을 말할 수 있다.

    ## 흔한 막힘

    - join()을 구분자가 아니라 리스트 뒤에 붙여 씀
    - find()가 못 찾을 때 -1을 준다는 점을 놓침
    - 메서드 결과를 변수에 다시 담지 않음
