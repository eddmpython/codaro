...
def _(mo):
    ...
    오늘의 핵심 범위는 `공백 제거`, `대소문자 변환`, `분리`, `결합`, `치환`, `검색`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `문자열 메서드`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    아래 코드는 `replace` 결과를 저장하지 않아 원본이 그대로입니다. 새 값을 변수에 담으세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `입력 문장을 검색용 slug로 정리하세요. 앞뒤 공백 제거, 소문자 변환, 공백을 하이픈으로 바꾸는 순서입니다.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
