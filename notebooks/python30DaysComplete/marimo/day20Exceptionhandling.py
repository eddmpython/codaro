...
def _(mo):
    ...
    오늘의 핵심 범위는 `try`, `except`, `구체 예외`, `else`, `finally`, `raise`, `예외 메시지`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `예외 처리`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    아래 코드는 `KeyError`가 나는데 `ValueError`만 잡고 있습니다. 실제 예외 타입에 맞게 고치세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `0으로 나누는 경우를 잡아 `None`을 반환하는 함수를 작성하세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
