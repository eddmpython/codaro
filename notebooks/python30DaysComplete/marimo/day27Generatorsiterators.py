...
def _(mo):
    ...
    이 노트북은 `curricula/python/30days/day27_제너레이터와이터레이터.yaml` YAML을 원본으로 생성했습니다. 위에서 아래로 읽고 실행하되, 연습 셀은 일부러 비워둔 공간입니다.

    ## 오늘의 목표

    yield, generator, iter(), next()로 값을 필요할 때 하나씩 만드는 흐름을 이해합니다.

    ## 왜 중요한가

    큰 데이터를 한꺼번에 만들지 않고 필요한 만큼만 처리하는 방식은 효율적인 프로그램의 기초입니다.

    ## 생각 모델

    제너레이터는 멈췄다가 다음 값을 요청받으면 이어서 실행되는 함수입니다.
...
def _(mo):
    ...
    - 코딩 전: return으로 끝나는 함수와 yield로 잠시 멈추는 함수를 비교합니다.
    - 실행 중: next()를 여러 번 호출하며 함수가 어디서 이어지는지 확인합니다.
    - 연습 초점: 숫자 흐름, 필터링 흐름, 파일 줄 처리처럼 하나씩 꺼내는 예제를 만듭니다.
    - 막히면: StopIteration이 나오면 더 꺼낼 값이 남아 있는지 확인합니다.
    """)
    return
...
def _(mo):
    ...
    - yield와 return의 차이를 설명할 수 있다.
    - iter()와 next()로 값을 하나씩 꺼낼 수 있다.
    - 이터레이터 규약이 무엇을 약속하는지 말할 수 있다.

    ## 흔한 막힘

    - 제너레이터를 리스트처럼 바로 값 묶음으로 생각함
    - next() 이후 상태가 이어진다는 점을 놓침
    - StopIteration을 오류로만 해석함
