...
def _(mo):
    ...
    오늘의 핵심 범위는 `yield`, `제너레이터`, `next`, `iter`, `이터레이터 프로토콜`, `지연 평가`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `제너레이터와 이터레이터`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    0부터 `limit - 1`까지 짝수만 내보내는 `evenNumbers`를 완성하세요.
...
def _(mo):
    ...
    아래 코드는 제너레이터를 한 번 소비한 뒤 다시 쓰려고 합니다. 새 제너레이터를 만들거나 리스트로 보관하세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `제너레이터로 특정 길이 이상의 단어만 하나씩 내보내세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
