...
def _(mo):
    ...
    이 노트북은 `curricula/python/30days/day15_함수기초.yaml` YAML을 원본으로 생성했습니다. 위에서 아래로 읽고 실행하되, 연습 셀은 일부러 비워둔 공간입니다.

    ## 오늘의 목표

    def, 매개변수, 인자, return으로 코드를 재사용 가능한 함수로 묶습니다.

    ## 왜 중요한가

    함수는 같은 로직을 여러 번 쓰게 만들고, 프로그램을 작은 단위로 이해하게 해줍니다.

    ## 생각 모델

    함수는 입력을 받아 작업한 뒤 결과를 돌려주는 작은 기계입니다.
...
def _(mo):
    ...
    - 코딩 전: 함수 이름, 입력 이름, 돌려줄 값을 먼저 나누어 적습니다.
    - 실행 중: 함수를 정의하는 셀과 호출하는 셀이 다르다는 점을 확인합니다.
    - 연습 초점: 반복해서 쓰던 계산이나 메시지 만들기를 함수로 바꿉니다.
    - 막히면: None이 나오면 return이 있는지, print()만 한 것은 아닌지 확인합니다.
    """)
    return
...
def _(mo):
    ...
    - return 값이 호출 위치로 돌아온다는 점을 말할 수 있다.
    - docstring으로 함수의 의도를 짧게 남길 수 있다.

    ## 흔한 막힘

    - 함수를 정의만 하고 호출하지 않음
    - print()와 return을 혼동함
    - 매개변수 이름과 호출 값을 같은 것으로 착각함
