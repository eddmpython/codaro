...
def _(mo):
    ...
    오늘의 핵심 범위는 `안전한 읽기`, `키 목록`, `값 목록`, `키-값 순회`, `갱신`, `삭제`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `딕셔너리 메서드`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    아래 코드는 `keys()` 결과에서 값 3000을 찾으려고 합니다. 값을 보려면 `values()`를 쓰세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `없는 키를 안전하게 읽고, 여러 설정을 한 번에 업데이트하세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
