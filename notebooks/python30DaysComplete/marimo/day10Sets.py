...
def _(mo):
    ...
    **오늘의 초점**: 중복 제거와 집합 연산으로 데이터를 비교한다.
...
def _(mo):
    ...
    사용자 목록, 태그, 권한처럼 중복보다 포함 관계가 중요한 데이터가 많다. 집합은 이런 비교를 짧고 정확하게 표현한다.
    
    ## 생각 모델
    
    집합은 순서보다 소속 여부가 중요한 묶음이다. 어떤 값이 들어 있는지, 두 묶음이 어떻게 겹치는지가 핵심이다.
    
    ## 자주 하는 실수
    
    - `{}`를 빈 집합으로 착각하기
    - 집합에 순서가 있다고 기대하기
    - `remove`와 `discard`의 에러 차이를 모르는 것
    """)
    return
...
def _(mo):
    ...
    1. `중복 제거`를 한 문장으로 설명할 수 있는가?
    2. `소속 검사`를 잘못 쓰면 어떤 결과나 에러가 날 수 있는가?
...
def _(mo):
    ...
    오늘의 핵심 범위는 `중복 제거`, `소속 검사`, `add`, `discard`, `합집합`, `교집합`, `차집합`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `집합`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    | 줄 | 코드 | 역할 |
    |---:|---|---|
    | 1 | `webUsers = {"Ari", "Bo", "Choi"}` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 2 | `appUsers = {"Bo", "Dana"}` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 3 | `sharedUsers = webUsers & appUsers` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 4 | `sharedUsers` | 마지막 표현식이거나 호출입니다. 실행 결과를 관찰해 상태를 확인합니다. |
    """)
    return
...
def _(mo):
    ...
    `set([1, 1, 2, 3])`의 결과에서 1은 몇 번 남을까요?
...
def _(mo):
    ...
    `paidUsers`에는 있지만 `activeUsers`에는 없는 사용자를 찾으세요.
...
def _(mo):
    ...
    아래 코드는 빈 집합을 만들려다 빈 딕셔너리를 만들었습니다. `set()`을 사용하세요.
...
def _(mo):
    ...
    두 그룹에 모두 속한 사람과 어느 한쪽에만 속한 전체 사람을 구하세요.
...
def _(mo):
    ...
    **랩 목표**: 이벤트 참가자와 설문 응답자의 교집합, 전체 고유 인원을 구하세요.
...
def _(mo):
    ...
    이벤트 참가자와 설문 응답자의 교집합, 전체 고유 인원을 구하세요.
...
def _(mo):
    ...
    **변형 과제**: 참가했지만 설문에 응답하지 않은 사람을 찾아보세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `두 그룹에 모두 속한 사람과 어느 한쪽에만 속한 전체 사람을 구하세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
    3. **역문제**: 결과값을 먼저 정하고, 그 결과가 나오도록 입력 데이터를 설계합니다.
    4. **오류 만들기**: 오늘의 자주 하는 실수 중 하나를 일부러 만들고, 에러 이름이나 잘못된 결과를 기록합니다.
    5. **설명하기**: 중복 제거, 소속 검사, add 중 하나를 비전공자에게 설명하는 3문장 메모를 씁니다.
