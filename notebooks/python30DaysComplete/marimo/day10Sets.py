...
def _(mo):
    ...
    오늘은 `중복 제거`, `소속 검사`, `add`, `discard`, `합집합`, `교집합`, `차집합`만 집중합니다. 한 번에 너무 많이 배우면 어디서 막혔는지 찾기 어렵기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 내용은 `집합`입니다. 예제 셀을 실행하기 전에 아래 순서로 천천히 따라가 봅니다.
...
def _(mo):
    ...
    자주 하는 실수는 빈 집합을 만들려다 빈 딕셔너리를 만들었습니다. `set()`을 사용하세요.
...
def _(mo):
    ...
    2. **값 바꾸기**: 비슷한 문제 `두 그룹에 모두 속한 사람과 어느 한쪽에만 속한 전체 사람을 구하세요.`에서 숫자나 문자열을 하나 바꾸고 확인 코드도 함께 고칩니다.
