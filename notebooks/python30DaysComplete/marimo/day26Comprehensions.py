...
def _(mo):
    ...
    오늘은 `리스트 컴프리헨션`, `조건 필터`, `딕셔너리 컴프리헨션`, `집합 컴프리헨션`, `중첩 컴프리헨션`만 집중합니다. 한 번에 너무 많이 배우면 어디서 막혔는지 찾기 어렵기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 내용은 `컴프리헨션`입니다. 예제 셀을 실행하기 전에 아래 순서로 천천히 따라가 봅니다.
...
def _(mo):
    ...
    2. **값 바꾸기**: 비슷한 문제 `컴프리헨션으로 짝수의 제곱만 모으세요.`에서 숫자나 문자열을 하나 바꾸고 확인 코드도 함께 고칩니다.
