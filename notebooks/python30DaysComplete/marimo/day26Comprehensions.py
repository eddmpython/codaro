...
def _(mo):
    ...
    **오늘의 초점**: 반복과 조건을 짧고 읽기 쉬운 데이터 변환식으로 쓴다.
...
def _(mo):
    ...
    컴프리헨션은 반복문을 무조건 줄이는 기술이 아니라, '무엇을 만들 것인가'가 분명할 때 코드를 선명하게 만드는 기술이다.
    
    ## 생각 모델
    
    컴프리헨션은 결과 모양을 맨 앞에 쓰고, 뒤에서 데이터를 하나씩 꺼내며, 필요하면 조건으로 거른다.
    
    ## 자주 하는 실수
    
    - 너무 복잡한 로직을 한 줄에 우겨 넣기
    - 결과 표현식과 반복 변수를 혼동하기
    - 딕셔너리 컴프리헨션에서 키와 값 위치를 바꾸기
    """)
    return
...
def _(mo):
    ...
    1. `리스트 컴프리헨션`를 한 문장으로 설명할 수 있는가?
    2. `조건 필터`를 잘못 쓰면 어떤 결과나 에러가 날 수 있는가?
...
def _(mo):
    ...
    오늘의 핵심 범위는 `리스트 컴프리헨션`, `조건 필터`, `딕셔너리 컴프리헨션`, `집합 컴프리헨션`, `중첩 컴프리헨션`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `컴프리헨션`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    | 줄 | 코드 | 역할 |
    |---:|---|---|
    | 1 | `prices = [1000, 2500, 4000]` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 2 | `salePrices = [int(price * 0.9) for price in prices]` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 3 | `salePrices` | 마지막 표현식이거나 호출입니다. 실행 결과를 관찰해 상태를 확인합니다. |
    """)
    return
...
def _(mo):
    ...
    `[num * num for num in range(4)]`의 결과를 예측하세요.
...
def _(mo):
    ...
    `words`에서 길이가 5 이상인 단어만 소문자로 모으세요.
...
def _(mo):
    ...
    아래 딕셔너리 컴프리헨션은 키와 값을 반대로 만들었습니다. 단어를 키, 길이를 값으로 고치세요.
...
def _(mo):
    ...
    **랩 목표**: 주문 목록에서 결제 완료된 주문만 골라 상품명과 할인 금액 딕셔너리를 만드세요.
...
def _(mo):
    ...
    주문 목록에서 결제 완료된 주문만 골라 상품명과 할인 금액 딕셔너리를 만드세요.
...
def _(mo):
    ...
    **변형 과제**: 같은 로직을 일반 for문으로 다시 작성해 가독성을 비교해보세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `컴프리헨션으로 짝수의 제곱만 모으세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
    3. **역문제**: 결과값을 먼저 정하고, 그 결과가 나오도록 입력 데이터를 설계합니다.
    4. **오류 만들기**: 오늘의 자주 하는 실수 중 하나를 일부러 만들고, 에러 이름이나 잘못된 결과를 기록합니다.
    5. **설명하기**: 리스트 컴프리헨션, 조건 필터, 딕셔너리 컴프리헨션 중 하나를 비전공자에게 설명하는 3문장 메모를 씁니다.
...
def _(mo):
    ...
    - 오늘 내가 배운 핵심은 `컴프리헨션`이고, 한 문장으로 말하면:
    - 내가 고친 버그의 원인은:
