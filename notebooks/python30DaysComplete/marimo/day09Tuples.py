...
def _(mo):
    ...
    오늘의 핵심 범위는 `튜플 생성`, `불변성`, `패킹`, `언패킹`, `반환값 묶음`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `튜플`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    아래 코드는 튜플 값을 직접 바꾸려고 해서 실패합니다. 새 튜플을 만들어 해결하세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `튜플로 묶인 좌표를 언패킹하고, 두 좌표의 합을 구하세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
