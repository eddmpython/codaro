...
def _(mo):
    ...
    이 노트북은 `curricula/python/30days/day11_딕셔너리기초.yaml` YAML을 원본으로 생성했습니다. 위에서 아래로 읽고 실행하되, 연습 셀은 일부러 비워둔 공간입니다.

    ## 오늘의 목표

    딕셔너리로 키와 값을 묶고, 키로 값을 읽고 바꿉니다.

    ## 왜 중요한가

    현실 데이터는 이름:값 구조가 많습니다. 딕셔너리는 그런 데이터를 가장 직접적으로 표현합니다.

    ## 생각 모델

    딕셔너리는 이름표가 붙은 서랍입니다. 위치 번호가 아니라 키로 값을 찾습니다.
...
def _(mo):
    ...
    - 오늘은 일부러 쓰지 않는 개념: 딕셔너리 메서드, 직접 만드는 함수, import
...
def _(mo):
    ...
    - 코딩 전: 각 항목에서 콜론 왼쪽 키와 오른쪽 값을 분리해서 읽습니다.
    - 실행 중: 없는 키를 읽을 때와 새 키에 값을 넣을 때의 차이를 확인합니다.
    - 연습 초점: 학생, 상품, 설정처럼 속성이 있는 대상을 딕셔너리로 표현합니다.
    - 막히면: KeyError가 나면 실제 존재하는 키 목록을 먼저 출력합니다.
    """)
    return
...
def _(mo):
    ...
    - 딕셔너리를 만들고 키로 값을 읽을 수 있다.
    - 새 키를 추가하고 기존 값을 바꿀 수 있다.
    - 리스트 인덱스와 딕셔너리 키의 차이를 설명할 수 있다.
