...
app = marimo.App(app_title="Day 11. 딕셔너리 기초")
...
def _(mo):
    ...
    현실 데이터는 이름표가 붙은 값의 묶음인 경우가 많다. 딕셔너리는 사용자, 상품, 설정처럼 필드가 있는 데이터를 표현하는 핵심 구조다.
    
    ## 생각 모델
    
    딕셔너리는 사전처럼 키로 값을 찾는다. 리스트가 위치 번호로 찾는다면 딕셔너리는 의미 있는 이름으로 찾는다.
    
    ## 자주 하는 실수
    
    - 키 문자열 따옴표를 빼먹기
    - 없는 키를 바로 읽기
    - 리스트 인덱스와 딕셔너리 키를 섞어 생각하기
    """)
    return
...
def _(mo):
    ...
    1. `키와 값`를 한 문장으로 설명할 수 있는가?
    2. `값 읽기`를 잘못 쓰면 어떤 결과나 에러가 날 수 있는가?
...
def _(mo):
    ...
    오늘의 핵심 범위는 `키와 값`, `값 읽기`, `값 수정`, `새 키 추가`, `중첩 데이터의 시작`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `딕셔너리 기초`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    | 줄 | 코드 | 역할 |
    |---:|---|---|
    | 1 | `profile = {"name": "Jin", "level": 5}` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 2 | `profile["level"] = profile["level"] + 1` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 3 | `profile` | 마지막 표현식이거나 호출입니다. 실행 결과를 관찰해 상태를 확인합니다. |
    """)
    return
...
def _(mo):
    ...
    `book`에 `pages` 키를 추가하고 값은 320으로 넣으세요.
...
def _(mo):
    ...
    아래 코드는 없는 키를 읽어서 실패합니다. 존재하는 키 이름으로 고치세요.
...
def _(mo):
    ...
    **랩 목표**: 학생 한 명의 성적표를 딕셔너리로 만들고 평균 점수를 계산하세요.
...
def _(mo):
    ...
    학생 한 명의 성적표를 딕셔너리로 만들고 평균 점수를 계산하세요.
...
def _(mo):
    ...
    **변형 과제**: `report`에 `passed` 키를 추가하고 평균이 80 이상인지 저장해보세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `딕셔너리에 새 키를 추가하고 기존 값을 수정하세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
    3. **역문제**: 결과값을 먼저 정하고, 그 결과가 나오도록 입력 데이터를 설계합니다.
    4. **오류 만들기**: 오늘의 자주 하는 실수 중 하나를 일부러 만들고, 에러 이름이나 잘못된 결과를 기록합니다.
    5. **설명하기**: 키와 값, 값 읽기, 값 수정 중 하나를 비전공자에게 설명하는 3문장 메모를 씁니다.
...
def _(mo):
    ...
    - 오늘 내가 배운 핵심은 `딕셔너리 기초`이고, 한 문장으로 말하면:
    - 내가 고친 버그의 원인은:
