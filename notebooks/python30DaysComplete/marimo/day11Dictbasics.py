...
def _(mo):
    ...
    오늘은 `키와 값`, `값 읽기`, `값 수정`, `새 키 추가`, `중첩 데이터의 시작`만 집중합니다. 한 번에 너무 많이 배우면 어디서 막혔는지 찾기 어렵기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 내용은 `딕셔너리 기초`입니다. 예제 셀을 실행하기 전에 아래 순서로 천천히 따라가 봅니다.
...
def _(mo):
    ...
    자주 하는 실수는 없는 키를 읽어서 오류가 납니다. 존재하는 키 이름으로 고치세요.
...
def _(mo):
    ...
    2. **값 바꾸기**: 비슷한 문제 `딕셔너리에 새 키를 추가하고 기존 값을 수정하세요.`에서 숫자나 문자열을 하나 바꾸고 확인 코드도 함께 고칩니다.
