...
def _(mo):
    ...
    1. `변수 대입`를 한 문장으로 설명할 수 있는가?
    2. `정수와 실수`를 잘못 쓰면 어떤 결과나 에러가 날 수 있는가?
...
def _(mo):
    ...
    오늘의 핵심 범위는 `변수 대입`, `정수와 실수`, `문자열`, `불리언`, `타입 확인`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `변수와 데이터 타입`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    아래 코드는 숫자 문자열과 정수를 바로 더하려고 해서 실패합니다. 문자열을 정수로 바꾸세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `학습 시간을 분 단위에서 시간 단위로 바꾸세요. 나눗셈 결과는 실수입니다.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
    3. **역문제**: 결과값을 먼저 정하고, 그 결과가 나오도록 입력 데이터를 설계합니다.
    4. **오류 만들기**: 오늘의 자주 하는 실수 중 하나를 일부러 만들고, 에러 이름이나 잘못된 결과를 기록합니다.
    5. **설명하기**: 변수 대입, 정수와 실수, 문자열 중 하나를 비전공자에게 설명하는 3문장 메모를 씁니다.
