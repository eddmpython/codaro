...
def _(mo):
    ...
    이 노트북은 `curricula/python/30days/day02_변수와데이터타입.yaml` YAML을 원본으로 생성했습니다. 위에서 아래로 읽고 실행하되, 연습 셀은 일부러 비워둔 공간입니다.

    ## 오늘의 목표

    변수에 값을 담고, 값의 종류를 type()과 len()으로 확인합니다.

    ## 왜 중요한가

    프로그램은 값을 저장하고 다시 꺼내 쓰는 일의 반복입니다. 변수는 그 반복을 시작하는 첫 도구입니다.

    ## 생각 모델

    변수는 값에 붙이는 이름표입니다. 이름표를 바꾸면 다음 줄에서 꺼내 쓰는 값도 달라집니다.
...
def _(mo):
    ...
    - 오늘은 일부러 쓰지 않는 개념: 리스트, 딕셔너리, 튜플, 집합, 직접 만드는 함수, 클래스, import
...
def _(mo):
    ...
    - 코딩 전: 왼쪽 이름, 등호, 오른쪽 값을 나누어 읽고 어떤 값이 저장될지 예상합니다.
    - 실행 중: 같은 변수에 값을 다시 넣었을 때 이전 값이 어떻게 바뀌는지 확인합니다.
    - 연습 초점: 이름을 스스로 정하고 숫자, 문자열, 참/거짓 값을 각각 담아봅니다.
    - 막히면: NameError가 나면 변수 이름 철자가 앞뒤로 같은지 확인합니다.
    """)
    return
...
def _(mo):
    ...
    - 변수에 값을 저장하고 print()로 꺼내 볼 수 있다.
    - int, float, str, bool의 차이를 예로 설명할 수 있다.
    - type()과 len()이 무엇을 확인하는지 말할 수 있다.

    ## 흔한 막힘

    - 변수 이름을 저장할 때와 사용할 때 다르게 씀
    - 문자열 숫자와 실제 숫자를 혼동함
    - len()을 숫자 값에 바로 사용함
