...
def _(mo):
    ...
    2. `문자열 리터럴`를 잘못 쓰면 어떤 결과나 에러가 날 수 있는가?
...
def _(mo):
    ...
    오늘의 핵심 범위는 `코드 셀 실행`, `문자열 리터럴`, `숫자 표현식`, `결과 관찰`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `Hello World와 실행 감각`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    `answer`가 `Hello, Codaro`가 되도록 `...` 부분을 바꾸세요.
...
def _(mo):
    ...
    아래 코드는 문자열 따옴표가 빠져서 실패합니다. 값을 문자열로 고쳐 실행하세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `다른 인사 문장을 직접 만들어보세요. `intro`가 정확히 `I run Python today.`가 되어야 합니다.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
    3. **역문제**: 결과값을 먼저 정하고, 그 결과가 나오도록 입력 데이터를 설계합니다.
    4. **오류 만들기**: 오늘의 자주 하는 실수 중 하나를 일부러 만들고, 에러 이름이나 잘못된 결과를 기록합니다.
    5. **설명하기**: 코드 셀 실행, 문자열 리터럴, 숫자 표현식 중 하나를 비전공자에게 설명하는 3문장 메모를 씁니다.
