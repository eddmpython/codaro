...
def _(mo):
    ...
    오늘의 핵심 범위는 `for`, `while`, `range`, `누적`, `break`, `continue`, `중첩 반복`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `반복문`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `문자열 목록에서 길이가 5 이상인 단어만 모으세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
