...
def _(mo):
    ...
    오늘의 핵심 범위는 `with 문`, `__enter__`, `__exit__`, `contextlib`, `정리 보장`, `고급 문법 연결`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `고급 문법 종합`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    `TimerLabel` 컨텍스트 매니저의 `__enter__`가 라벨 문자열을 반환하도록 완성하세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `컨텍스트 매니저가 들어갈 때와 나올 때 상태를 바꾸도록 작성하세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
