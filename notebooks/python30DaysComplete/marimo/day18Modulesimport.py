...
def _(mo):
    ...
    오늘의 핵심 범위는 `import`, `from import`, `as 별칭`, `표준 라이브러리`, `모듈 네임스페이스`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `모듈과 import`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    아래 코드는 `random` 모듈을 가져오지 않고 사용합니다. import를 추가하세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 ``random`에 seed를 주고 1부터 6 사이의 숫자를 만드세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
