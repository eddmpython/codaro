...
def _(mo):
    ...
    오늘의 핵심 범위는 `산술 연산`, `나머지`, `비교 연산`, `논리 연산`, `멤버십 연산`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `연산자`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    `score`가 80 이상이고 `attendance`가 10 이상이면 통과입니다. `passed`를 완성하세요.
...
def _(mo):
    ...
    아래 코드는 할인 조건을 반대로 쓰고 있습니다. 20000원 이상일 때 할인되도록 고치세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `무료 배송 조건을 판단하세요. 주문 금액이 30000 이상이거나 VIP면 무료 배송입니다.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
    3. **역문제**: 결과값을 먼저 정하고, 그 결과가 나오도록 입력 데이터를 설계합니다.
    4. **오류 만들기**: 오늘의 자주 하는 실수 중 하나를 일부러 만들고, 에러 이름이나 잘못된 결과를 기록합니다.
    5. **설명하기**: 산술 연산, 나머지, 비교 연산 중 하나를 비전공자에게 설명하는 3문장 메모를 씁니다.
