...
def _(mo):
    ...
    오늘의 핵심 범위는 `기본 매개변수`, `키워드 인자`, `*args`, `**kwargs`, `lambda`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `함수 고급`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    여러 숫자를 받아 합계를 반환하는 `sumAll` 함수를 완성하세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `기본 배송비가 있는 결제 함수를 만들고 키워드 인자로 배송비를 바꿔보세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
