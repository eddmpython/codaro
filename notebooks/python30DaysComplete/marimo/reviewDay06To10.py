...
def _(mo):
    ...
    | 범위 | 주제 | 핵심 개념 |
    |---|---|---|
    | Day 06 | 문자열 메서드 | 공백 제거, 대소문자 변환, 분리, 결합 |
    | Day 07 | 리스트 기초 | 리스트 생성, 리스트 인덱싱, 리스트 슬라이싱, 값 변경 |
    | Day 08 | 리스트 메서드 | 추가, 삽입, 삭제, 정렬 |
    | Day 09 | 튜플 | 튜플 생성, 불변성, 패킹, 언패킹 |
    | Day 10 | 집합 | 중복 제거, 소속 검사, add, discard |
    """)
    return
