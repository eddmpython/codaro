...
def _(mo):
    ...
    오늘의 핵심 범위는 `if`, `elif`, `else`, `중첩 조건`, `조건 표현식`, `블록 들여쓰기`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `조건문`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    아래 코드는 높은 점수를 먼저 검사하지 않아 95점도 B가 됩니다. 조건 순서를 고치세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `점수에 따라 `excellent`, `pass`, `retry` 중 하나를 고르세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
