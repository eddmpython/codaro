...
def _(mo):
    ...
    오늘의 핵심 범위는 `지역 스코프`, `전역 스코프`, `nonlocal`, `global`, `클로저`, `상태 캡슐화`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `스코프와 클로저`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    `makeMultiplier`가 배수를 기억하는 함수를 반환하도록 완성하세요.
...
def _(mo):
    ...
    아래 코드는 바깥 함수의 `count`를 수정하려 하지만 `nonlocal`이 없어 실패합니다.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `클로저로 접두어를 기억하는 formatter를 만드세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
