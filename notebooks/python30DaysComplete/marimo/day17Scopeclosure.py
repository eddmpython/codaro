...
def _(mo):
    ...
    이 노트북은 `curricula/python/30days/day17_스코프와클로저.yaml` YAML을 원본으로 생성했습니다. 위에서 아래로 읽고 실행하되, 연습 셀은 일부러 비워둔 공간입니다.

    ## 오늘의 목표

    스코프, global, nonlocal, 클로저로 이름이 보이는 범위를 이해합니다.

    ## 왜 중요한가

    함수가 많아질수록 값이 어디서 왔고 어디까지 영향을 주는지 알아야 오류를 줄일 수 있습니다.

    ## 생각 모델

    스코프는 이름을 찾는 방입니다. 파이썬은 현재 방에서 찾고, 없으면 바깥 방을 확인합니다.
...
def _(mo):
    ...
    - 코딩 전: 각 변수 이름이 함수 안에서 만들어졌는지 밖에서 만들어졌는지 표시합니다.
    - 실행 중: 같은 이름이 안과 밖에 있을 때 어느 값이 출력되는지 확인합니다.
    - 연습 초점: 전역 변수를 직접 바꾸기보다 함수가 값을 받아 돌려주는 구조를 우선 사용합니다.
    - 막히면: UnboundLocalError가 나면 함수 안에서 같은 이름에 대입한 줄이 있는지 확인합니다.
    """)
    return
...
def _(mo):
    ...
    - 지역 스코프와 전역 스코프를 구분할 수 있다.
    - global과 nonlocal이 필요한 상황을 예로 설명할 수 있다.
    - 클로저가 바깥 함수의 값을 기억한다는 점을 말할 수 있다.

    ## 흔한 막힘

    - 함수 안 대입이 바깥 변수도 자동으로 바꾼다고 생각함
    - global을 습관적으로 사용함
    - 클로저의 기억되는 값을 출력 결과만으로 외움
