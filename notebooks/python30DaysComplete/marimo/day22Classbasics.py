...
def _(mo):
    ...
    **오늘의 초점**: 데이터와 동작을 함께 묶는 클래스를 만든다.
...
def _(mo):
    ...
    딕셔너리로도 데이터를 묶을 수 있지만, 그 데이터가 할 수 있는 동작까지 함께 묶고 싶을 때 클래스가 필요하다. 클래스는 같은 구조의 객체를 반복해서 만들 수 있게 해준다.
    
    ## 생각 모델
    
    클래스는 설계도이고 인스턴스는 설계도로 만든 실제 물건이다. `self`는 지금 동작 중인 그 물건 자신을 가리킨다.
...
def _(mo):
    ...
    1. `class`를 한 문장으로 설명할 수 있는가?
    2. `__init__`를 잘못 쓰면 어떤 결과나 에러가 날 수 있는가?
...
def _(mo):
    ...
    오늘의 핵심 범위는 `class`, `__init__`, `self`, `인스턴스`, `속성`, `메서드`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `클래스 기초`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    | 줄 | 코드 | 역할 |
    |---:|---|---|
    | 1 | `class Timer:` | 데이터와 동작을 묶을 새 타입의 설계도를 만듭니다. |
    | 2 | `    def __init__(self, minutes):` | 재사용할 동작에 이름을 붙입니다. 입력과 반환값을 함께 생각합니다. |
    | 3 | `        self.minutes = minutes` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 4 | ` ` | 읽기 좋게 구획을 나누는 빈 줄입니다. |
    | 5 | `    def add(self, amount):` | 재사용할 동작에 이름을 붙입니다. 입력과 반환값을 함께 생각합니다. |
    | 6 | `        self.minutes = self.minutes + amount` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 7 | `        return self.minutes` | 함수 호출자에게 돌려줄 결과를 정합니다. |
    | 8 | ` ` | 읽기 좋게 구획을 나누는 빈 줄입니다. |
    | 9 | `timer = Timer(25)` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 10 | `timer.add(5)` | 마지막 표현식이거나 호출입니다. 실행 결과를 관찰해 상태를 확인합니다. |
    """)
    return
...
def _(mo):
    ...
    서로 다른 인스턴스는 속성 값을 공유할까요? 아래 결과를 확인하세요.
...
def _(mo):
    ...
    `Book` 클래스에 제목과 페이지 수를 저장하고, `isLong` 메서드를 완성하세요.
...
def _(mo):
    ...
    아래 메서드는 `self`를 빼먹었습니다. 인스턴스 메서드 첫 번째 매개변수로 `self`를 넣으세요.
...
def _(mo):
    ...
    작은 `Habit` 클래스를 만들고 완료 횟수를 1 늘리는 메서드를 작성하세요.
...
def _(mo):
    ...
    **랩 목표**: 할 일 하나를 표현하는 `Task` 클래스를 만들고 완료 처리 메서드를 작성하세요.
...
def _(mo):
    ...
    할 일 하나를 표현하는 `Task` 클래스를 만들고 완료 처리 메서드를 작성하세요.
...
def _(mo):
    ...
    **변형 과제**: `rename` 메서드를 추가해 제목을 바꿀 수 있게 해보세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `작은 `Habit` 클래스를 만들고 완료 횟수를 1 늘리는 메서드를 작성하세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
    3. **역문제**: 결과값을 먼저 정하고, 그 결과가 나오도록 입력 데이터를 설계합니다.
    4. **오류 만들기**: 오늘의 자주 하는 실수 중 하나를 일부러 만들고, 에러 이름이나 잘못된 결과를 기록합니다.
    5. **설명하기**: class, __init__, self 중 하나를 비전공자에게 설명하는 3문장 메모를 씁니다.
...
def _(mo):
    ...
    - 오늘 내가 배운 핵심은 `클래스 기초`이고, 한 문장으로 말하면:
    - 내가 고친 버그의 원인은:
