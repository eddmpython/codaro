...
def _(mo):
    ...
    오늘은 `class`, `__init__`, `self`, `인스턴스`, `속성`, `메서드`만 집중합니다. 한 번에 너무 많이 배우면 어디서 막혔는지 찾기 어렵기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 내용은 `클래스 기초`입니다. 예제 셀을 실행하기 전에 아래 순서로 천천히 따라가 봅니다.
...
def _(mo):
    ...
    `Book` 클래스에 제목과 페이지 수를 저장하고, `isLong` 메서드를 실행해 확인하세요.
...
def _(mo):
    ...
    2. **값 바꾸기**: 비슷한 문제 `작은 `Habit` 클래스를 만들고 완료 횟수를 1 늘리는 메서드를 작성하세요.`에서 숫자나 문자열을 하나 바꾸고 확인 코드도 함께 고칩니다.
