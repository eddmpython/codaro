...
def _(mo):
    ...
    오늘의 핵심 범위는 `문자열 연결`, `f-string`, `이스케이프`, `여러 줄 문자열`, `포함 여부`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `문자열 기초`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    아래 코드는 숫자와 문자열을 `+`로 바로 합치고 있습니다. f-string으로 고치세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `이름과 남은 미션 수를 사용해 `Noa has 3 missions left.` 문장을 만드세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
    3. **역문제**: 결과값을 먼저 정하고, 그 결과가 나오도록 입력 데이터를 설계합니다.
    4. **오류 만들기**: 오늘의 자주 하는 실수 중 하나를 일부러 만들고, 에러 이름이나 잘못된 결과를 기록합니다.
    5. **설명하기**: 문자열 연결, f-string, 이스케이프 중 하나를 비전공자에게 설명하는 3문장 메모를 씁니다.
