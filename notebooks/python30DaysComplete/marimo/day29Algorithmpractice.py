...
def _(mo):
    ...
    **오늘의 초점**: 문제를 작게 나누고 자료구조를 선택해 해결한다.
...
def _(mo):
    ...
    알고리즘은 어려운 공식 암기가 아니라 문제를 다루는 습관이다. 입력 크기, 필요한 결과, 반복되는 구조를 보면 어떤 도구가 맞는지 판단할 수 있다.
    
    ## 생각 모델
    
    먼저 손으로 작은 예제를 풀고, 그 과정을 코드로 옮긴다. 코드부터 쓰면 문제의 규칙을 놓치기 쉽다.
    
    ## 자주 하는 실수
    
    - 작은 예제를 손으로 풀지 않고 코드부터 쓰기
    - 경계 인덱스를 하나씩 틀리기
    - 빈 입력 리스트를 고려하지 않기
    """)
    return
...
def _(mo):
    ...
    1. `문제 분석`를 한 문장으로 설명할 수 있는가?
    2. `정렬`를 잘못 쓰면 어떤 결과나 에러가 날 수 있는가?
...
def _(mo):
    ...
    - Day 28: 제목을 보지 않고 핵심 문법 하나와 실수 하나를 떠올린다.
...
def _(mo):
    ...
    오늘의 핵심 범위는 `문제 분석`, `정렬`, `선형 탐색`, `딕셔너리 빈도`, `재귀`, `복잡도 감각`입니다. 범위를 좁게 잡는 이유는 하나의 노트북에서 너무 많은 문법을 섞으면 실패 원인을 찾기 어려워지기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 개념은 `알고리즘 연습`입니다. 예제 셀을 실행하기 전에 아래 순서로 머릿속 실행을 해봅니다.
...
def _(mo):
    ...
    | 줄 | 코드 | 역할 |
    |---:|---|---|
    | 1 | `numbers = [4, 1, 7, 1, 4, 4]` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 2 | `counts = {}` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 3 | `for num in numbers:` | 묶음에서 값을 하나씩 꺼내 같은 규칙을 적용합니다. |
    | 4 | `    counts[num] = counts.get(num, 0) + 1` | 계산 결과나 데이터를 이름에 연결합니다. |
    | 5 | `counts` | 마지막 표현식이거나 호출입니다. 실행 결과를 관찰해 상태를 확인합니다. |
    """)
    return
...
def _(mo):
    ...
    최댓값을 찾는 반복문에서 `best`가 언제 바뀌는지 추적하세요.
...
def _(mo):
    ...
    리스트에서 가장 자주 등장한 값을 반환하는 `mostCommon` 함수를 완성하세요.
...
def _(mo):
    ...
    아래 이진 탐색은 `right` 초기값이 범위를 벗어납니다. 마지막 인덱스로 고치세요.
...
def _(mo):
    ...
    두 포인터 방식으로 정렬된 리스트에서 목표 합을 찾으세요.
...
def _(mo):
    ...
    **랩 목표**: 주문 금액 리스트에서 기준 금액 이상인 주문의 개수와 총액을 반환하는 함수를 만드세요.
...
def _(mo):
    ...
    주문 금액 리스트에서 기준 금액 이상인 주문의 개수와 총액을 반환하는 함수를 만드세요.
...
def _(mo):
    ...
    **변형 과제**: 같은 함수를 컴프리헨션과 `sum`으로 다시 작성해보세요.
...
def _(mo):
    ...
    2. **변형**: 전이 연습 `두 포인터 방식으로 정렬된 리스트에서 목표 합을 찾으세요.`에서 숫자나 문자열을 하나 바꾸고 `assert`도 함께 고칩니다.
    3. **역문제**: 결과값을 먼저 정하고, 그 결과가 나오도록 입력 데이터를 설계합니다.
    4. **오류 만들기**: 오늘의 자주 하는 실수 중 하나를 일부러 만들고, 에러 이름이나 잘못된 결과를 기록합니다.
    5. **설명하기**: 문제 분석, 정렬, 선형 탐색 중 하나를 비전공자에게 설명하는 3문장 메모를 씁니다.
...
def _(mo):
    ...
    - 오늘 내가 배운 핵심은 `알고리즘 연습`이고, 한 문장으로 말하면:
    - 내가 고친 버그의 원인은:
