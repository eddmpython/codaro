...
def _(mo):
    ...
    이 노트북은 `curricula/python/30days/day29_알고리즘연습.yaml` YAML을 원본으로 생성했습니다. 위에서 아래로 읽고 실행하되, 연습 셀은 일부러 비워둔 공간입니다.

    ## 오늘의 목표

    정렬, 탐색, 재귀 문제를 작은 단계로 나누어 해결합니다.

    ## 왜 중요한가

    알고리즘 연습은 문법을 문제 해결 절차로 바꾸는 훈련입니다.

    ## 생각 모델

    알고리즘은 정답 코드가 아니라 입력을 출력으로 바꾸는 단계 목록입니다.
...
def _(mo):
    ...
    - 코딩 전: 입력 예시, 원하는 출력, 직접 손으로 푸는 절차를 먼저 적습니다.
    - 실행 중: 작은 입력부터 실행하고 중간 값을 출력해 절차가 맞는지 확인합니다.
    - 연습 초점: 정렬 기준 바꾸기, 원하는 값 찾기, 재귀의 종료 조건을 집중 연습합니다.
    - 막히면: 재귀가 끝나지 않으면 종료 조건과 더 작은 문제로 줄어드는 줄을 확인합니다.
    """)
    return
...
def _(mo):
    ...
    - 문제를 입력, 처리, 출력으로 나눌 수 있다.
    - 정렬과 탐색의 기본 절차를 설명할 수 있다.
    - 재귀 함수의 종료 조건과 재귀 호출을 구분할 수 있다.
