...
def _(mo):
    ...
    오늘은 `문제 분석`, `정렬`, `선형 탐색`, `딕셔너리 빈도`, `재귀`, `복잡도 감각`만 집중합니다. 한 번에 너무 많이 배우면 어디서 막혔는지 찾기 어렵기 때문입니다.
...
def _(mo):
    ...
    오늘 핵심 내용은 `알고리즘 연습`입니다. 예제 셀을 실행하기 전에 아래 순서로 천천히 따라가 봅니다.
...
def _(mo):
    ...
    리스트에서 가장 자주 등장한 값을 반환하는 `mostCommon` 함수를 실행해 확인하세요.
...
def _(mo):
    ...
    2. **값 바꾸기**: 비슷한 문제 `두 포인터 방식으로 정렬된 리스트에서 목표 합을 찾으세요.`에서 숫자나 문자열을 하나 바꾸고 확인 코드도 함께 고칩니다.
