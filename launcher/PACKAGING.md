...
`launcher/PRD.md`가 설치기와 로컬 배포 운영체제의 관점이라면, 이 문서는 wheel과 release artifact 관점을 맡는다.
...
- GitHub Releases와 optional PyPI는 어떻게 역할을 나눌 것인가
...
- GitHub Releases는 product control plane이자 launcher artifact plane이다
- PyPI는 Python developer install과 생태계 검색을 위한 보조 배포 채널이다. Codaro.exe는 PyPI latest를 해석하지 않고 GitHub Releases의 release-manifest.json이 핀한 `codaro` wheel만 설치한다.
...
5. tests
6. wheel을 GitHub Release asset으로 업로드
7. release manifest 생성
8. GitHub Release에 manifest와 launcher/python runtime asset 업로드
9. GitHub Release published 이벤트 후 `.github/workflows/publish.yaml`이 같은 version의 wheel/sdist를 PyPI Trusted Publisher로 업로드
...
4. wheel을 GitHub Release asset으로 업로드
5. release manifest에 정확한 wheel과 sha256 반영

## PyPI Publish Policy

PyPI는 제품 launcher update에 필수 조건이 아니다. 역할은 developer install, third-party automation package 재사용, Python 생태계 검색/배포 채널이다.

운영 원칙:

- GitHub Actions Trusted Publisher 사용
- PyPI API token을 long-lived secret로 저장하지 않는다
- GitHub Release가 published 된 뒤 publish workflow가 실행된다
- PyPI publisher 값은 project `codaro`, owner `eddmpython`, repository `codaro`, workflow `publish.yaml`, environment `pypi`와 일치한다
...
## Why Not Just a GitHub Pages Tag

GitHub Pages에서 최신 tag 문자열만 제공하는 방식은 update trigger로는 쓸 수 있지만 설치 계약으로는 부족하다.

필요한 정보:

- release channel과 prerelease 허용 여부
- 최소 launcher version
- managed Python runtime URL과 sha256
- backend wheel URL과 sha256
- optional bundle wheel URL과 sha256
- rollback target

따라서 public site는 사용자가 다운로드 링크를 찾는 표면으로 두고, launcher는 GitHub Release의 `release-manifest.json`을 기준으로 설치한다. GitHub Pages가 필요해지면 tag만이 아니라 manifest URL을 담은 static index를 제공해야 한다.
...
editor frontend는 기본적으로 `codaro` wheel 내부 `codaro/webBuild`로 들어간다. editor zip은 `editor.source: "archive"`를 쓰는 legacy/internal smoke release에서만 별도 asset으로 둔다.

현재 product release에서는 wheel을 같은 GitHub Release asset으로 두고, manifest가 해당 wheel URL과 sha256을 가리킨다.
...
- `.github/workflows/product-release.yml`
  ...
  - upload exact wheel, manifest, launcher, runtime assets to GitHub Release

- `.github/workflows/publish.yaml`
  - trigger on published GitHub Release or manual dispatch
  - build editor
  - stage built-in curricula into the package
  - build wheel and sdist
  - verify package payload and wheel install
  - publish to PyPI through Trusted Publisher environment `pypi`

- `bundle-release.yml`
  - bundle wheel release asset
  - compatibility test
...
- PyPI publish는 `.github/workflows/publish.yaml`에서 분리 운영한다. 제품 launcher update는 여전히 GitHub Release manifest 기준이다.
...
- 다음 normal `vX.Y.Z` 태그에서 wheel build, release manifest 생성, GitHub Release asset 업로드가 한 번에 성공하는지 확인한다
- compiled dependency wheel과 platform tag 조합 정책을 정한다

## Verification Left

- 최신 GitHub Release에 `release-manifest.json`, exact backend wheel, `python-runtime-win-x64.zip`, launcher checksum, SBOM이 함께 붙는지 확인
