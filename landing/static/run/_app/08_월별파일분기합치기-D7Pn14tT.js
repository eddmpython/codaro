...
assessment:
  ...
  masteryVariants:
  - id: xlwings_08-monthly-workbook-admission-mastery
    mode: mastery
    unseen: true
    ...
    - prepare-inputs
    - summary
    title: 월별 파일 병합 입력의 기간·schema·중복 감사하기
    subtitle: 새 입력으로 핵심 분석 재현
    goal: 잘못된 기간·schema drift·중복 file hash를 차단한다.
    why: worked example을 복사하지 않고 새 레코드에서 같은 분석 판단을 재현해야 개념 숙달을 확인할 수 있습니다.
    explanation: 브라우저의 격리된 Python Worker가 보이지 않던 정상·경계·오류 입력으로 함수를 다시 호출합니다.
    tips: &id001
    - 파일명보다 content hash와 명시적 month를 입장 기준으로 사용하세요.
    - 병합 전 column 이름과 순서를 정확히 비교하세요.
    exercise:
      prompt: audit_monthly_files(files, expected_columns, period_start, period_end)를 완성하세요.
      starterCode: |-
      ...
      hints: *id001
    check:
      id: python.xlwings.xlwings_08.monthly-workbook-admission.mastery.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_08.monthly-workbook-admission.mastery.behavior.v1.fixture
      ...
      packageAssets: []
      payload:
        entry: audit_monthly_files
        cases:
        - id: accepts-monthly-inputs
          arguments:
          - value:
            - name: 01.xlsx
              hash: a
  ...
  transferVariants:
  - id: xlwings_08-monthly-merge-reconciliation-transfer
    mode: transfer
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - xlwings_08-monthly-workbook-admission-mastery
    title: 월별 병합 결과의 행 수와 금액 대조하기
    subtitle: 다른 업무 문맥으로 판단 전이
    goal: source별 contribution과 전체 합계를 검증한다.
    why: 같은 판단을 다른 데이터 계약과 업무 질문으로 옮겨야 특정 예제 암기와 전이를 구분할 수 있습니다.
    explanation: 숙달 근거가 저장되면 별도 확인 클릭 없이 열리는 새 문맥 과제입니다.
    tips: &id002
    - source별 row count와 total을 더해 expected를 만드세요.
    - 결과 manifest에 모든 source content hash를 포함하세요.
    exercise:
      prompt: reconcile_monthly_merge(sources, result)를 완성하세요.
      starterCode: |-
      ...
      hints: *id002
    check:
      id: python.xlwings.xlwings_08.monthly-merge-reconciliation.transfer.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_08.monthly-merge-reconciliation.transfer.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: reconcile_monthly_merge
        cases:
        - id: accepts-reconciled-merge
          arguments:
          - value:
            - hash: a
              rows: 2
        ...
        normalizeReturnPaths: []
  retrievalVariants:
  - id: xlwings_08-monthly-merge-recall-retrieval
    mode: retrieval
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - xlwings_08-monthly-merge-reconciliation-transfer
    title: 월별 Excel 병합 원칙 회상하기
    subtitle: 7일 뒤 기준을 기억에서 복원
    goal: 입장·병합·대조 증거를 기억에서 복원한다.
    ...
    exercise:
      prompt: choose_monthly_merge_evidence(stage)를 완성해 action, evidence, risk를 반환하세요.
      starterCode: |-
        def choose_monthly_merge_evidence(stage):
            raise NotImplementedError
      solution: |
        def choose_monthly_merge_evidence(stage):
            table = {'admit': {'action': 'validate hash period schema', 'evidence': 'source admission ledger', 'risk': 'duplicate or stale input'}, 'merge': {'action': 'preserve source identity per row', 'evidence': 'row provenance', 'risk': 'untraceable records'}, 'reconcile': {'action': 'compare rows totals source hashes', 'evidence': 'merge report', 'risk': 'silent omission'}}
            if stage not in table:
                raise ValueError('unknown stage')
            return table[stage]
      hints: *id003
    check:
      id: python.xlwings.xlwings_08.monthly-merge-recall.retrieval.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_08.monthly-merge-recall.retrieval.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: choose_monthly_merge_evidence
        cases:
        - id: recalls-admit
          arguments:
          - value: admit
          expectedReturn:
            action: validate hash period schema
            evidence: source admission ledger
            risk: duplicate or stale input
        - id: recalls-merge
          arguments:
          - value: merge
          expectedReturn:
            action: preserve source identity per row
            evidence: row provenance
            risk: untraceable records
        - id: recalls-reconcile
          arguments:
          - value: reconcile
          expectedReturn:
            action: compare rows totals source hashes
            evidence: merge report
            risk: silent omission
        expectedPaths: []
        normalizeReturnPaths: []
