...
- id: hello_world
  ...
  goal: Hello World에서 print() 입력값이 출력 영역에 어떻게 표시되는지 확인한다.\r
...
\r
  ...
  exercise:
    ...
    predict:\r
      prompt: 셀을 실행하면 출력 영역에 무엇이 표시될까요?\r
      expectedValue: Hello World\r
    variations:\r
    - prompt: 작은따옴표 대신 큰따옴표로 바꿔도 같은 결과가 나오는지 확인하세요.\r
      starterCode: print("Hello Codaro")\r
      parameterization: 따옴표 종류 변경 - 동일 출력 검증\r
    - prompt: print() 안에 쉼표로 두 값을 넣으면 어떻게 표시되는지 확인하세요.\r
      starterCode: print('Hello', 'World')\r
      parameterization: 인자 개수 변경 - 공백 구분자 검증\r
  check:
...
- id: print_korean\r
  ...
  goal: 한글 출력하기에서 print() 입력값이 출력 영역에 어떻게 표시되는지 확인한다.\r
  why: 출력 확인은 코드가 의도대로 실행됐는지 가장 작게 점검하는 방법입니다.\r
  explanation: |-\r
...
- id: print_multiple\r
  ...
  goal: 여러 줄 출력하기에서 여러 print() 호출의 출력 순서와 줄 수를 확인한다.\r
  why: 출력 확인은 코드가 의도대로 실행됐는지 가장 작게 점검하는 방법입니다.\r
...
- id: print_newline\r
  ...
  structuredPrimary: true\r
  subtitle: \\n으로 한 번에 여러 줄 출력\r
  goal: 줄바꿈 문자에서 print() 입력값이 출력 영역에 어떻게 표시되는지 확인한다.\r
  why: 출력 확인은 코드가 의도대로 실행됐는지 가장 작게 점검하는 방법입니다.\r
  explanation: |-\r
...
- id: comment_single\r
  ...
  goal: 한 줄 주석에서 print() 입력값이 출력 영역에 어떻게 표시되는지 확인한다.\r
  why: 출력 확인은 코드가 의도대로 실행됐는지 가장 작게 점검하는 방법입니다.\r
  explanation: |-\r
...
- id: comment_multi\r
  ...
  goal: 여러 줄 설명 메모에서 여러 print() 호출의 출력 순서와 줄 수를 확인한다.\r
  why: 출력 확인은 코드가 의도대로 실행됐는지 가장 작게 점검하는 방법입니다.\r
  explanation: |-\r
...
\r
  ...
  exercise:
    ...
    - 실행 뒤 줄 수와 순서가 유지되고, 수정한 줄만 의도대로 바뀌었는지 보세요.\r
    predict:\r
      prompt: 셀을 실행하면 출력 영역과 마지막 표현식에 무엇이 표시될까요?\r
      expectedValue: (직접 실행해 본 결과를 적어주세요)\r
  check:
    type: outputExact
    evidence: practice
    outputExact: |-
...
- id: print_numbers\r
  ...
  goal: 숫자 출력하기에서 print() 입력값이 출력 영역에 어떻게 표시되는지 확인한다.\r
  why: 출력 확인은 코드가 의도대로 실행됐는지 가장 작게 점검하는 방법입니다.\r
  explanation: |-\r
...
\r
  ...
  exercise:
    ...
    hints:\r
    - 바꿀 지점은 print() 괄호 안의 숫자 리터럴입니다.\r
    - 실행 뒤 출력된 숫자가 입력한 숫자와 정확히 같은지 보세요.\r
    predict:\r
      prompt: 이 print() 호출의 출력 영역에는 어떤 숫자가 표시될까요?\r
      expectedValue: 123\r
  check:
    type: outputExact
    evidence: practice
...
- id: print_calculation\r
  ...
  goal: 계산 결과 출력하기에서 print() 입력값이 출력 영역에 어떻게 표시되는지 확인한다.\r
  why: 출력 확인은 코드가 의도대로 실행됐는지 가장 작게 점검하는 방법입니다.\r
  ...
  exercise:
    ...
    hints:\r
    - 바꿀 지점은 print() 안의 숫자, 연산자, 괄호 위치에서 찾으세요.\r
    - 실행 뒤 출력 숫자를 직접 계산한 값과 비교하세요.\r
    predict:\r
      prompt: 이 계산식의 결과는 어떤 숫자일까요? 직접 계산해 본 뒤 실행과 비교하세요.\r
      expectedValue: (계산 결과 숫자를 적어주세요)\r
  check:
    type: outputExact
    evidence: practice
    outputExact: '42'
...
- id: notebook_expression\r
  ...
  goal: 마지막 줄 값 확인하기에서 셀 마지막 표현식이 결과로 표시되는 흐름을 확인한다.\r
  ...
  exercise:
    ...
    predict:\r
      prompt: 셀 마지막 줄의 문자열은 어떻게 표시될까요?\r
      expectedValue: '''Hello Notebook'''\r
  check:
    type: outputExact
    evidence: practice
...
- id: practice\r
  ...
  goal: Day 1 종합 복습에서 print() 입력값이 출력 영역에 어떻게 표시되는지 확인한다.\r
  why: 출력 확인은 코드가 의도대로 실행됐는지 가장 작게 점검하는 방법입니다.\r
