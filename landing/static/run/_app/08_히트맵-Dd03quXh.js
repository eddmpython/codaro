...
\r
      m10\r
    hints:\r
    - 바꿀 지점은 두 \`range(...)\`의 반복 횟수와 \`storeLocations\`에 넣는 위도, 경도 범위입니다.\r
    - 실행 뒤 반복 횟수, 누적값, 만들어진 리스트 길이가 바뀐 입력을 반영하는지 보세요.\r
  check:\r
...
assessment:\r
  schemaVersion: 1\r
  performanceClaim: 웹에서는 외부 패키지 없이 분석 판단과 데이터 계약을 검증하고, 실제 패키지 API와 산출물은 lesson Run 및 Local 실습 증거로 분리합니다.\r
  tierParity:\r
  ...
  masteryVariants:\r
  - id: folium_08-heatmap-weight-data-evidence-mastery\r
    mode: mastery\r
    unseen: true\r
    ...
    - step1_import\r
    - practice\r
    title: 공간 heatmap 데이터 증거 만들기\r
    subtitle: 새 입력으로 핵심 분석 재현\r
    goal: 점 밀도와 weight 합을 인구·수요로 오해하지 않는가에 답하기 전에 usable·excluded 분모와 축 범위를 고정한다.\r
    why: worked example을 복사하지 않고 새 레코드에서 같은 분석 판단을 재현해야 개념 숙달을 확인할 수 있습니다.\r
    explanation: 브라우저의 격리된 Python Worker가 보이지 않던 정상·경계·오류 입력으로 함수를 다시 호출합니다.\r
    tips: &id001\r
    - 차트에 들어가지 않은 NULL 행도 excludedCount로 보존하세요.\r
    - 축 범위와 그룹별 표본 수 없이 모양만 해석하지 마세요.\r
    exercise:\r
      prompt: prepare_heatmap_weight(rows)를 완성해 차트에 실제 사용된 행 수, 제외 수, 그룹 수, 두 축 범위를 반환하세요.\r
      starterCode: |-\r
        def prepare_heatmap_weight(rows):\r
            raise NotImplementedError\r
      solution: |\r
        def prepare_heatmap_weight(rows):\r
            required = ['longitude', 'latitude', 'weightBand']\r
            if any(not set(required) <= set(row) for row in rows):\r
                raise ValueError("chart schema mismatch")\r
            usable = [row for row in rows if all(row[name] is not None for name in required)]\r
            groups = {}\r
            group_field = 'weightBand'\r
            for row in usable:\r
                key = "all" if group_field is None else str(row[group_field])\r
                groups[key] = groups.get(key, 0) + 1\r
            x_values = [row['longitude'] for row in usable]\r
            y_values = [row['latitude'] for row in usable]\r
            return {\r
                "usableCount": len(usable),\r
                "excludedCount": len(rows) - len(usable),\r
                "groupCounts": {key: groups[key] for key in sorted(groups)},\r
                "xExtent": None if not x_values else [min(x_values), max(x_values)],\r
                "yExtent": None if not y_values else [min(y_values), max(y_values)],\r
            }\r
      hints: *id001\r
    check:\r
      id: python.folium.folium_08.heatmap-weight-data-evidence.mastery.behavior.v1\r
      version: 1\r
      kind: behavior\r
      strength: strong\r
      executor: browser-worker\r
      timeoutMs: 8000\r
      fixtureId: python.folium.folium_08.heatmap-weight-data-evidence.mastery.behavior.v1.fixture\r
      ...
      packageAssets: []\r
      payload:\r
        entry: prepare_heatmap_weight\r
        cases:\r
        - id: summarizes-visible-data\r
          arguments:\r
          - value:\r
            - longitude: 127.0\r
              latitude: 37.5\r
  ...
  transferVariants:\r
  - id: folium_08-heatmap-weight-encoding-transfer-transfer\r
    mode: transfer\r
    unseen: true\r
    claimScope: portable-concept\r
    reviewStatus: machine-verified-pending-independent-review\r
    sourceSectionIds:\r
    - folium_08-heatmap-weight-data-evidence-mastery\r
    title: 공간 heatmap 인코딩 계약을 새 문맥에 전이하기\r
    subtitle: 다른 업무 문맥으로 판단 전이\r
    goal: '다음 새 문맥에서도 mark·axis·transform·interaction 책임을 재현한다: 호출 위치의 건수와 severity weight를 구분한 공간 heatmap을 만든다.'\r
    why: 같은 판단을 다른 데이터 계약과 업무 질문으로 옮겨야 특정 예제 암기와 전이를 구분할 수 있습니다.\r
    explanation: 숙달 근거가 저장되면 별도 확인 클릭 없이 열리는 새 문맥 과제입니다.\r
    tips: &id002\r
    - 표현 mark만 맞아도 충분하지 않습니다. 축·그룹·변환을 함께 검사하세요.\r
    - description은 보이지 않는 사용자와 차트를 열 수 없는 상황의 핵심 증거입니다.\r
    exercise:\r
      prompt: audit_heatmap_weight(candidate)를 완성해 주어진 차트 사양의 오류와 기대 encoding을 반환하세요.\r
      starterCode: |-\r
        def audit_heatmap_weight(candidate):\r
            raise NotImplementedError\r
      solution: |\r
        def audit_heatmap_weight(candidate):\r
            expected = {'mark': 'heatmap', 'x': 'longitude', 'y': 'latitude', 'group': 'weightBand', 'transforms': ['normalize-radius', 'validate-coordinates', 'validate-weight'], 'interaction': 'pan-zoom'}\r
            errors = []\r
            for name in ["mark", "x", "y", "group", "transforms", "interaction"]:\r
                actual = sorted(candidate.get(name, [])) if name == "transforms" else candidate.get(name)\r
                if actual != expected[name]:\r
                    errors.append(name)\r
            if not str(candidate.get("description", "")).strip():\r
                errors.append("description")\r
            return {"valid": not errors, "errors": errors, "encoding": expected}\r
      hints: *id002\r
    check:\r
      id: python.folium.folium_08.heatmap-weight-encoding-transfer.transfer.behavior.v1\r
      version: 1\r
      kind: behavior\r
      strength: strong\r
      executor: browser-worker\r
      timeoutMs: 8000\r
      fixtureId: python.folium.folium_08.heatmap-weight-encoding-transfer.transfer.behavior.v1.fixture\r
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=\r
      fixture:\r
        directories:\r
        - input\r
      ...
      packageAssets: []\r
      payload:\r
        entry: audit_heatmap_weight\r
        cases:\r
        - id: accepts-complete-encoding\r
          arguments:\r
          - value:\r
              mark: heatmap\r
              x: longitude\r
              ...
              transforms:\r
              - normalize-radius\r
              - validate-coordinates\r
              - validate-weight\r
              interaction: pan-zoom\r
              description: 호출 위치의 건수와 severity weight를 구분한 공간 heatmap을 만든다\r
          expectedReturn:\r
            valid: true\r
            errors: []\r
            encoding:\r
              ...
              - validate-weight\r
              interaction: pan-zoom\r
        - id: reports-misleading-encoding\r
          arguments:\r
          - value:\r
              mark: table\r
              x: latitude\r
        ...
        normalizeReturnPaths: []\r
  retrievalVariants:\r
  - id: folium_08-heatmap-weight-interpretation-retrieval-retrieval\r
    mode: retrieval\r
    unseen: true\r
    claimScope: portable-concept\r
    reviewStatus: machine-verified-pending-independent-review\r
    sourceSectionIds:\r
    - folium_08-heatmap-weight-encoding-transfer-transfer\r
    title: 공간 heatmap 해석 위험 회상하기\r
    subtitle: 7일 뒤 기준을 기억에서 복원\r
    goal: 점 밀도와 weight 합을 인구·수요로 오해하지 않는가 다시 판단할 때 차트 선택과 증거 한계를 구분한다.\r
    why: 시간을 둔 뒤 핵심 기준을 다시 구성해야 단기 모방과 장기 기억을 구분할 수 있습니다.\r
    explanation: 전이 과제를 통과한 지 7일 뒤 자동으로 열리며, worked example은 다시 노출하지 않습니다.\r
    tips: &id003\r
    - 차트가 보여주는 패턴과 인과 주장을 구분하세요.\r
    - 축·분모·결측·표본 수 중 무엇이 해석을 바꾸는지 명시하세요.\r
    exercise:\r
      prompt: choose_heatmap_weight(situation)를 완성해 encoding, evidence, risk를 반환하세요.\r
      starterCode: |-\r
        def choose_heatmap_weight(situation):\r
            raise NotImplementedError\r
      solution: |\r
        def choose_heatmap_weight(situation):\r
            table = {'event-density': {'encoding': 'unweighted heatmap', 'evidence': 'point count', 'risk': 'population exposure'}, 'severity-weight': {'encoding': 'weighted heatmap', 'evidence': 'weight definition', 'risk': 'one extreme point'}, 'precise-location': {'encoding': 'clustered markers', 'evidence': 'individual records', 'risk': 'heatmap hides points'}}\r
            if situation not in table:\r
                raise ValueError('unknown situation')\r
            return table[situation]\r
      hints: *id003\r
    check:\r
      id: python.folium.folium_08.heatmap-weight-interpretation-retrieval.retrieval.behavior.v1\r
      version: 1\r
      kind: behavior\r
      strength: strong\r
      executor: browser-worker\r
      timeoutMs: 8000\r
      fixtureId: python.folium.folium_08.heatmap-weight-interpretation-retrieval.retrieval.behavior.v1.fixture\r
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=\r
      fixture:\r
        directories:\r
        - input\r
      ...
      packageAssets: []\r
      payload:\r
        entry: choose_heatmap_weight\r
        cases:\r
        - id: recalls-event-density\r
          arguments:\r
          - value: event-density\r
          expectedReturn:\r
            encoding: unweighted heatmap\r
            evidence: point count\r
            risk: population exposure\r
        - id: recalls-severity-weight\r
          arguments:\r
          - value: severity-weight\r
          expectedReturn:\r
            encoding: weighted heatmap\r
            evidence: weight definition\r
            risk: one extreme point\r
        - id: rejects-unknown\r
          arguments:\r
          - value: unknown\r
