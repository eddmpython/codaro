...
assessment:
  ...
  masteryVariants:
  - id: playwright_06-storage-state-redaction-mastery
    mode: mastery
    unseen: true
    ...
    sourceSectionIds:
    - cookie-state
    - reuse-state-completion
    title: 로그인 storage state에서 비밀값 제거하기
    subtitle: 새 입력으로 핵심 분석 재현
    goal: cookie와 local storage의 민감한 이름을 보존 가능한 redacted 값으로 바꾼다.
    ...
    exercise:
      prompt: redact_storage_state(state, secret_names)를 완성하세요.
      starterCode: |-
      ...
      hints: *id001
    check:
      id: python.playwright.playwright_06.storage-state-redaction.mastery.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.playwright.playwright_06.storage-state-redaction.mastery.behavior.v1.fixture
      ...
      packageAssets: []
      payload:
        entry: redact_storage_state
        cases:
        - id: redacts-cookie-and-local-storage
          arguments:
          - value:
              cookies:
              - name: session
          ...
          expectedReturn:
            ...
            - origin: https://app.test
              ...
              - name: theme
                value: dark
            redactedCount: 2
        - id: matches-secret-names-case-insensitively
          arguments:
          - value:
              cookies:
              - name: Auth
  ...
  transferVariants:
  - id: playwright_06-auth-state-scope-audit-transfer
    mode: transfer
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - playwright_06-storage-state-redaction-mastery
    title: 새 인증 상태의 origin·만료 범위 감사하기
    subtitle: 다른 업무 문맥으로 판단 전이
    goal: 허용하지 않은 origin, 만료 cookie, 과도한 수명을 함께 차단한다.
    ...
    exercise:
      prompt: audit_auth_state(state, allowed_origins, now, maximum_lifetime)를 완성하세요.
      starterCode: |-
      ...
      hints: *id002
    check:
      id: python.playwright.playwright_06.auth-state-scope-audit.transfer.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.playwright.playwright_06.auth-state-scope-audit.transfer.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: audit_auth_state
        cases:
        - id: accepts-scoped-fresh-state
          arguments:
          - value:
              cookies:
              - name: session
        ...
        normalizeReturnPaths: []
  retrievalVariants:
  - id: playwright_06-auth-storage-recall-retrieval
    mode: retrieval
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - playwright_06-auth-state-scope-audit-transfer
    title: 인증 storage 관리 원칙 회상하기
    subtitle: 7일 뒤 기준을 기억에서 복원
    goal: 저장·공유·폐기 단계의 비밀 보호 증거를 선택한다.
    ...
    exercise:
      prompt: choose_auth_storage_action(situation)를 완성해 action, evidence, risk를 반환하세요.
      starterCode: |-
        def choose_auth_storage_action(situation):
            raise NotImplementedError
      solution: |
        def choose_auth_storage_action(situation):
            table = {'create': {'action': 'local isolated profile login', 'evidence': 'origin and expiry audit', 'risk': 'credential capture'}, 'share-report': {'action': 'redact named secrets', 'evidence': 'redaction count and manifest', 'risk': 'unknown secret field'}, 'finish': {'action': 'delete state and revoke session', 'evidence': 'absence and server invalidation', 'risk': 'orphan session'}}
            if situation not in table:
                raise ValueError('unknown situation')
            return table[situation]
      hints: *id003
    check:
      id: python.playwright.playwright_06.auth-storage-recall.retrieval.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.playwright.playwright_06.auth-storage-recall.retrieval.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: choose_auth_storage_action
        cases:
        - id: recalls-create
          arguments:
          - value: create
          expectedReturn:
            action: local isolated profile login
            evidence: origin and expiry audit
            risk: credential capture
        - id: recalls-share-report
          arguments:
          - value: share-report
          expectedReturn:
            action: redact named secrets
            evidence: redaction count and manifest
            risk: unknown secret field
        - id: rejects-unknown
          arguments:
          - value: unknown
