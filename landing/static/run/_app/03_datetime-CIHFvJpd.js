...
- id: workflow_validation
  ...
  goal: 기준일을 고정한 마감 상태 계산이 assert 세 줄을 모두 통과하는지 실행으로 확인한다.
  ...
  exercise:
    prompt: |-
      이 셀은 고칠 곳이 없습니다. 기준일이나 마감일을 바꾸면 아래 assert 세 줄이 깨지므로 그대로 실행하세요.

      기준일 2026년 5월 24일을 기준으로 마감이 지난 것 1건, 사흘 안쪽 2건, 그 뒤 1건으로 갈리고 마지막 줄 taskSchedule이 그대로 표시되므로 아래 한 줄이 나와야 합니다.
      [{'task': 'invoiceClose', 'due': datetime.date(2026, 5, 20), 'daysLeft': -4, 'status': 'overdue'}, {'task': 'releaseReview', 'due': datetime.date(2026, 5, 24), 'daysLeft': 0, 'status': 'dueSoon'}, {'task': 'securityAudit', 'due': datetime.date(2026, 5, 27), 'daysLeft': 3, 'status': 'dueSoon'}, {'task': 'quarterReport', 'due': datetime.date(2026, 6, 2), 'daysLeft': 9, 'status': 'scheduled'}]
    starterCode: |-
      referenceDate = date(2026, 5, 24)
      taskRows = [
          {"task": "invoiceClose", "due": "2026-05-20"},
    ...
    solution: |-
      ...
      taskSchedule
    hints:
    - 코드를 바꾸지 말고 그대로 실행합니다. assert 세 줄이 아무 말 없이 지나가면 계산이 기대대로 된 것입니다.
    - '정답 형태: starterCode 그대로 실행, 마지막 줄 taskSchedule 이 네 건의 dict 목록으로 표시됩니다.'
  check:
    type: outputExact
    evidence: practice
    outputExact: "[{'task': 'invoiceClose', 'due': datetime.date(2026, 5, 20), 'daysLeft': -4, 'status': 'overdue'}, {'task': 'releaseReview', 'due': datetime.date(2026, 5, 24), 'daysLeft': 0, 'status': 'dueSoon'}, {'task': 'securityAudit', 'due': datetime.date(2026, 5, 27), 'daysLeft': 3, 'status': 'dueSoon'}, {'task': 'quarterReport', 'due': datetime.date(2026, 6, 2), 'daysLeft': 9, 'status': 'scheduled'}]"
    resultCheck: "출력이 정확히 일치해야 합니다: [{'task': 'invoiceClose', 'due': datetime.date(2026, 5, 20), 'daysLeft': -4, 'status': 'overdue'}, {'task': 'releaseReview', 'due': datetime.date(2026, 5, 24), 'daysLeft': 0, 'status': 'dueSoon'}, {'task': 'securityAudit', 'due': datetime.date(2026, 5, 27), 'daysLeft': 3, 'status': 'dueSoon'}, {'task': 'quarterReport', 'due': datetime.date(2026, 6, 2), 'daysLeft': 9, 'status': 'scheduled'}]"
- id: practice
