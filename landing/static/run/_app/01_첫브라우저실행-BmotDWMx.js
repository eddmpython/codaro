...
assessment:
  ...
  masteryVariants:
  - id: playwright_01-navigation-result-audit-mastery
    mode: mastery
    unseen: true
    ...
    - page-title
    - completion-object
    title: 첫 탐색의 status·origin·redirect 증거 판정하기
    subtitle: 새 입력으로 핵심 분석 재현
    goal: 응답 연쇄에서 최종 URL과 HTTP 실패, origin 이탈을 함께 찾는다.
    ...
    exercise:
      prompt: audit_navigation(responses, allowed_origins)를 완성하세요.
      starterCode: |-
      ...
      hints: *id001
    check:
      id: python.playwright.playwright_01.navigation-result-audit.mastery.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.playwright.playwright_01.navigation-result-audit.mastery.behavior.v1.fixture
      ...
      packageAssets: []
      payload:
        entry: audit_navigation
        cases:
        - id: accepts-allowed-redirect
          arguments:
          - value:
          ...
          expectedReturn:
            ...
            hops: 2
            failures: []
        - id: reports-origin-and-http
          arguments:
          - value:
            - url: https://example.test/start
              status: 302
            - url: https://login.test/fail
              status: 503
          - value:
            - https://example.test
          expectedReturn:
            ok: false
            finalUrl: https://login.test/fail
            finalStatus: 503
            hops: 2
            failures:
            - http-503
            - origin
        - id: rejects-empty-chain
          arguments:
          - value: []
          - value:
            - https://example.test
  ...
  transferVariants:
  - id: playwright_01-semantic-page-probe-transfer
    mode: transfer
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - playwright_01-navigation-result-audit-mastery
    title: 새 페이지에 의미 기반 첫 상태 점검 전이하기
    subtitle: 다른 업무 문맥으로 판단 전이
    goal: 제목·주요 landmark·console 오류로 탐색 직후 상태를 요약한다.
    ...
    exercise:
      prompt: summarize_page_probe(probe)를 완성하세요.
      starterCode: |-
      ...
      hints: *id002
    check:
      id: python.playwright.playwright_01.semantic-page-probe.transfer.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.playwright.playwright_01.semantic-page-probe.transfer.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: summarize_page_probe
        cases:
        - id: accepts-semantic-shell
          arguments:
          - value:
              title: Orders
              mainLandmarks: 1
          ...
          expectedReturn:
            ...
            title: Orders
            severeConsole: []
        - id: reports-empty-shell-and-console
          arguments:
          - value:
              title: ' '
              mainLandmarks: 0
        ...
        normalizeReturnPaths: []
  retrievalVariants:
  - id: playwright_01-navigation-evidence-recall-retrieval
    mode: retrieval
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - playwright_01-semantic-page-probe-transfer
    title: 첫 브라우저 탐색 증거 회상하기
    subtitle: 7일 뒤 기준을 기억에서 복원
    goal: 화면 표시·응답 실패·외부 이동을 서로 다른 증거로 판정한다.
    ...
    exercise:
      prompt: choose_navigation_evidence(situation)를 완성해 action, evidence, risk를 반환하세요.
      starterCode: |-
        def choose_navigation_evidence(situation):
            raise NotImplementedError
      solution: |
        def choose_navigation_evidence(situation):
            table = {'page-shell': {'action': 'assert title and main landmark', 'evidence': 'semantic snapshot', 'risk': 'empty client shell'}, 'redirect-chain': {'action': 'record every response', 'evidence': 'status and final origin', 'risk': 'unexpected identity provider'}, 'console-failure': {'action': 'collect severe console messages', 'evidence': 'message and page state', 'risk': 'silent boot failure'}}
            if situation not in table:
                raise ValueError('unknown situation')
            return table[situation]
      hints: *id003
    check:
      id: python.playwright.playwright_01.navigation-evidence-recall.retrieval.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.playwright.playwright_01.navigation-evidence-recall.retrieval.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: choose_navigation_evidence
        cases:
        - id: recalls-page-shell
          arguments:
          - value: page-shell
          expectedReturn:
            action: assert title and main landmark
            evidence: semantic snapshot
            risk: empty client shell
        - id: recalls-redirect-chain
          arguments:
          - value: redirect-chain
          expectedReturn:
            action: record every response
            evidence: status and final origin
            risk: unexpected identity provider
        - id: rejects-unknown
          arguments:
          - value: unknown
          expectedException: ValueError
