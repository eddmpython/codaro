...
assessment:
  ...
  masteryVariants:
  - id: xlwings_00-xlwings-capability-contract-mastery
    mode: mastery
    unseen: true
    ...
    - track-promise
    - readiness-checklist
    title: xlwings 작업의 Desktop Excel capability 계약 세우기
    subtitle: 새 입력으로 핵심 분석 재현
    goal: Desktop Excel runtime·interactive session·macro 요구를 명시적으로 분류한다.
    why: worked example을 복사하지 않고 새 레코드에서 같은 분석 판단을 재현해야 개념 숙달을 확인할 수 있습니다.
    explanation: 브라우저의 격리된 Python Worker가 보이지 않던 정상·경계·오류 입력으로 함수를 다시 호출합니다.
    tips: &id001
    - 실제 Excel 자동화에 필요한 capability를 작업 요구에서 파생하세요.
    - 패키지 설치 여부와 Desktop Excel 존재 여부를 한 항목으로 뭉치지 마세요.
    exercise:
      prompt: plan_xlwings_runtime(requirements, capabilities)를 완성하세요.
      starterCode: |-
      ...
      hints: *id001
    check:
      id: python.xlwings.xlwings_00.xlwings-capability-contract.mastery.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_00.xlwings-capability-contract.mastery.behavior.v1.fixture
      ...
      packageAssets: []
      payload:
        entry: plan_xlwings_runtime
        cases:
        - id: keeps-concepts-on-web
          arguments:
          - value: {}
          - value: {}
          expectedReturn:
        ...
        - id: accepts-local-capabilities
          arguments:
          - value:
              desktopWorkbook: true
          - value:
              desktopExcel: true
              xlwings: true
          expectedReturn:
  ...
  transferVariants:
  - id: xlwings_00-automation-boundary-plan-transfer
    mode: transfer
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - xlwings_00-xlwings-capability-contract-mastery
    title: 새 Excel 업무의 Web·Local 경계 계획하기
    subtitle: 다른 업무 문맥으로 판단 전이
    goal: 판단 단계와 외부 효과 단계를 분리해 실행 그래프를 만든다.
    why: 같은 판단을 다른 데이터 계약과 업무 질문으로 옮겨야 특정 예제 암기와 전이를 구분할 수 있습니다.
    explanation: 숙달 근거가 저장되면 별도 확인 클릭 없이 열리는 새 문맥 과제입니다.
    tips: &id002
    - 순수 판단 단계는 다운로드 없이 Web에서 연습 가능하게 두세요.
    - 외부 효과 단계마다 재개방·로그·스크린샷 같은 증거를 요구하세요.
    exercise:
      prompt: split_excel_workflow(steps)를 완성하세요.
      starterCode: |-
      ...
      hints: *id002
    check:
      id: python.xlwings.xlwings_00.automation-boundary-plan.transfer.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_00.automation-boundary-plan.transfer.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: split_excel_workflow
        cases:
        - id: splits-contract-and-artifact
          arguments:
          - value:
            - id: validate
              effect: pure
            - id: write
              effect: write-workbook
              evidence: reopen
          expectedReturn:
            webSteps:
            - validate
        ...
        normalizeReturnPaths: []
  retrievalVariants:
  - id: xlwings_00-xlwings-runtime-recall-retrieval
    mode: retrieval
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - xlwings_00-automation-boundary-plan-transfer
    title: xlwings 실행 계층 원칙 회상하기
    subtitle: 7일 뒤 기준을 기억에서 복원
    goal: Web 판단과 Local Excel 효과를 구분한다.
    ...
    exercise:
      prompt: choose_xlwings_runtime_evidence(stage)를 완성해 action, evidence, risk를 반환하세요.
      starterCode: |-
        def choose_xlwings_runtime_evidence(stage):
            raise NotImplementedError
      solution: |
        def choose_xlwings_runtime_evidence(stage):
            table = {'web': {'action': 'validate portable automation contract', 'evidence': 'behavior cases', 'risk': 'concept tied to one machine'}, 'local': {'action': 'control identified Excel session', 'evidence': 'process and workbook identity', 'risk': 'wrong instance'}, 'artifact': {'action': 'save reopen and reconcile workbook', 'evidence': 'artifact manifest', 'risk': 'successful call with wrong file'}}
            if stage not in table:
                raise ValueError('unknown stage')
            return table[stage]
      hints: *id003
    check:
      id: python.xlwings.xlwings_00.xlwings-runtime-recall.retrieval.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_00.xlwings-runtime-recall.retrieval.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: choose_xlwings_runtime_evidence
        cases:
        - id: recalls-web
          arguments:
          - value: web
          expectedReturn:
            action: validate portable automation contract
            evidence: behavior cases
            risk: concept tied to one machine
        - id: recalls-local
          arguments:
          - value: local
          expectedReturn:
            action: control identified Excel session
            evidence: process and workbook identity
            risk: wrong instance
        - id: recalls-artifact
          arguments:
          - value: artifact
          expectedReturn:
            action: save reopen and reconcile workbook
            evidence: artifact manifest
            risk: successful call with wrong file
        expectedPaths: []
        normalizeReturnPaths: []
