...
assessment:
  ...
  masteryVariants:
  - id: xlwings_05-formula-style-audit-mastery
    mode: mastery
    unseen: true
    ...
    - formula-write
    - summary
    title: 수식과 서식의 계산·표시 계약 감사하기
    subtitle: 새 입력으로 핵심 분석 재현
    goal: 수식 누락·하드코딩·표시 형식 누락을 찾는다.
    why: worked example을 복사하지 않고 새 레코드에서 같은 분석 판단을 재현해야 개념 숙달을 확인할 수 있습니다.
    explanation: 브라우저의 격리된 Python Worker가 보이지 않던 정상·경계·오류 입력으로 함수를 다시 호출합니다.
    tips: &id001
    - formula 문자열과 계산된 value를 서로 다른 증거로 다루세요.
    - number format은 업무 단위 계약의 일부로 검사하세요.
    exercise:
      prompt: audit_formula_cells(cells, required_formulas, required_formats)를 완성하세요.
      starterCode: |-
      ...
      hints: *id001
    check:
      id: python.xlwings.xlwings_05.formula-style-audit.mastery.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_05.formula-style-audit.mastery.behavior.v1.fixture
      ...
      packageAssets: []
      payload:
        entry: audit_formula_cells
        cases:
        - id: accepts-formula-and-format
          arguments:
          - value:
            - address: B2
              formula: =SUM(A1:A2)
  ...
  transferVariants:
  - id: xlwings_05-formula-result-reconciliation-transfer
    mode: transfer
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - xlwings_05-formula-style-audit-mastery
    title: 재계산된 수식 결과를 원본 데이터와 대조하기
    subtitle: 다른 업무 문맥으로 판단 전이
    goal: stale calculation과 수치 오차를 분리해 찾는다.
    why: 같은 판단을 다른 데이터 계약과 업무 질문으로 옮겨야 특정 예제 암기와 전이를 구분할 수 있습니다.
    explanation: 숙달 근거가 저장되면 별도 확인 클릭 없이 열리는 새 문맥 과제입니다.
    tips: &id002
    - Excel calculation 완료 여부와 value 정확성을 별도 검사하세요.
    - 업무 원본에서 다시 계산한 expected value를 사용하세요.
    exercise:
      prompt: reconcile_formula_results(cells, expected, tolerance=0.01)를 완성하세요.
      starterCode: |-
      ...
      hints: *id002
    check:
      id: python.xlwings.xlwings_05.formula-result-reconciliation.transfer.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_05.formula-result-reconciliation.transfer.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: reconcile_formula_results
        cases:
        - id: accepts-calculated-result
          arguments:
          - value:
            - address: B2
              value: 100
        ...
        normalizeReturnPaths: []
  retrievalVariants:
  - id: xlwings_05-formula-style-recall-retrieval
    mode: retrieval
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - xlwings_05-formula-result-reconciliation-transfer
    title: Excel 수식·서식 검증 원칙 회상하기
    subtitle: 7일 뒤 기준을 기억에서 복원
    goal: 수식·계산값·표시 단위를 구분한다.
    ...
    exercise:
      prompt: choose_formula_evidence(stage)를 완성해 action, evidence, risk를 반환하세요.
      starterCode: |-
        def choose_formula_evidence(stage):
            raise NotImplementedError
      solution: |
        def choose_formula_evidence(stage):
            table = {'formula': {'action': 'inspect exact formula text', 'evidence': 'formula map', 'risk': 'hardcoded result'}, 'value': {'action': 'force calculation and reconcile', 'evidence': 'calculated value report', 'risk': 'stale cache'}, 'format': {'action': 'verify number format', 'evidence': 'display contract', 'risk': 'wrong unit interpretation'}}
            if stage not in table:
                raise ValueError('unknown stage')
            return table[stage]
      hints: *id003
    check:
      id: python.xlwings.xlwings_05.formula-style-recall.retrieval.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_05.formula-style-recall.retrieval.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      payload:
        ...
        - id: recalls-formula
          ...
          - value: formula
          expectedReturn:
            action: inspect exact formula text
            evidence: formula map
            risk: hardcoded result
        - id: recalls-value
          arguments:
          - value: value
          expectedReturn:
            action: force calculation and reconcile
            evidence: calculated value report
            risk: stale cache
        - id: recalls-format
          arguments:
          - value: format
          expectedReturn:
            action: verify number format
            evidence: display contract
            risk: wrong unit interpretation
        expectedPaths: []
        normalizeReturnPaths: []
