...
assessment:
  ...
  masteryVariants:
  - id: xlwings_10-udf-purity-audit-mastery
    mode: mastery
    unseen: true
    ...
    - define-udf
    - summary
    title: UDF 가격 계산기의 순수성·결정성 계약 감사하기
    subtitle: 새 입력으로 핵심 분석 재현
    goal: 외부 상태·가변 시간·오류 은폐를 차단한다.
    why: worked example을 복사하지 않고 새 레코드에서 같은 분석 판단을 재현해야 개념 숙달을 확인할 수 있습니다.
    explanation: 브라우저의 격리된 Python Worker가 보이지 않던 정상·경계·오류 입력으로 함수를 다시 호출합니다.
    tips: &id001
    - UDF는 같은 입력에 같은 출력을 주는 순수 계산으로 제한하세요.
    - 오류를 빈 셀로 숨기지 말고 typed error로 드러내세요.
    exercise:
      prompt: audit_udf_contract(contract)를 완성하세요.
      starterCode: |-
      ...
      hints: *id001
    check:
      id: python.xlwings.xlwings_10.udf-purity-audit.mastery.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_10.udf-purity-audit.mastery.behavior.v1.fixture
  ...
  transferVariants:
  - id: xlwings_10-price-udf-cases-transfer
    mode: transfer
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - xlwings_10-udf-purity-audit-mastery
    title: 가격 UDF의 할인·세금 경계값 전이 검증하기
    subtitle: 다른 업무 문맥으로 판단 전이
    goal: Decimal 문자열 입력으로 반올림 규칙을 결정론적으로 적용한다.
    why: 같은 판단을 다른 데이터 계약과 업무 질문으로 옮겨야 특정 예제 암기와 전이를 구분할 수 있습니다.
    explanation: 숙달 근거가 저장되면 별도 확인 클릭 없이 열리는 새 문맥 과제입니다.
    tips: &id002
    - binary float 대신 Decimal 문자열 변환으로 금액을 계산하세요.
    - 할인율 범위와 음수 금액을 함수 경계에서 거부하세요.
    exercise:
      prompt: calculate_price(base, discount_rate, tax_rate)를 완성하세요.
      starterCode: |-
      ...
      hints: *id002
    check:
      id: python.xlwings.xlwings_10.price-udf-cases.transfer.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_10.price-udf-cases.transfer.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: calculate_price
        cases:
        - id: calculates-discount-tax
          arguments:
          - value: 100
          - value: '0.1'
          - value: '0.1'
        ...
        normalizeReturnPaths: []
  retrievalVariants:
  - id: xlwings_10-udf-release-recall-retrieval
    mode: retrieval
    unseen: true
    claimScope: portable-concept
    reviewStatus: machine-verified-pending-independent-review
    sourceSectionIds:
    - xlwings_10-price-udf-cases-transfer
    title: Excel UDF 릴리스 원칙 회상하기
    subtitle: 7일 뒤 기준을 기억에서 복원
    goal: 순수성·경계값·workbook 재계산 증거를 기억에서 복원한다.
    ...
    exercise:
      prompt: choose_udf_evidence(stage)를 완성해 action, evidence, risk를 반환하세요.
      starterCode: |-
        def choose_udf_evidence(stage):
            raise NotImplementedError
      solution: |
        def choose_udf_evidence(stage):
            table = {'contract': {'action': 'ban external effects and define schemas', 'evidence': 'UDF purity audit', 'risk': 'nondeterministic cells'}, 'cases': {'action': 'test numeric boundaries and errors', 'evidence': 'behavior case matrix', 'risk': 'silent financial error'}, 'workbook': {'action': 'recalculate and reconcile cell results', 'evidence': 'reopened formula values', 'risk': 'stale or unavailable UDF'}}
            if stage not in table:
                raise ValueError('unknown stage')
            return table[stage]
      hints: *id003
    check:
      id: python.xlwings.xlwings_10.udf-release-recall.retrieval.behavior.v1
      version: 1
      kind: behavior
      strength: strong
      executor: browser-worker
      timeoutMs: 8000
      fixtureId: python.xlwings.xlwings_10.udf-release-recall.retrieval.behavior.v1.fixture
      fixtureHash: sha256-5H2hz41NNRiQqR7gqqk7c7FuxPecIr+coT1+YyQEi2s=
      fixture:
        directories:
        - input
      ...
      packageAssets: []
      payload:
        entry: choose_udf_evidence
        cases:
        - id: recalls-contract
          arguments:
          - value: contract
          expectedReturn:
            action: ban external effects and define schemas
            evidence: UDF purity audit
            risk: nondeterministic cells
        - id: recalls-cases
          arguments:
          - value: cases
          expectedReturn:
            action: test numeric boundaries and errors
            evidence: behavior case matrix
            risk: silent financial error
        - id: recalls-workbook
          arguments:
          - value: workbook
          expectedReturn:
            action: recalculate and reconcile cell results
            evidence: reopened formula values
            risk: stale or unavailable UDF
        expectedPaths: []
        normalizeReturnPaths: []
