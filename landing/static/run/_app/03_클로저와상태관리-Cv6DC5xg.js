...
- id: closure_definition
  ...
  goal: 클로저 정의에서 추상화 패턴 흐름을 코드로 실행하고 결과를 확인한다.
  why: 함수 입력과 반환값을 작게 확인하면 이후 코드에서 같은 동작을 안전하게 재사용할 수 있습니다.
  explanation: |-
    클로저(Closure)는 자신이 정의된 스코프의 변수를 기억하는 함수입니다. 내부 함수가 외부 함수의 지역 변수를 참조하면, 외부 함수가 종료되어도 그 변수가 메모리에 유지됩니다. 이렇게 내부 함수에 의해 참조되는 외부 변수를 '자유 변수(free variable)'라고 합니다. 파이썬에서 클로저는 함수 객체의 __closure__ 속성에 셀(cell) 객체로 자유 변수를 저장합니다. 클로저는 데이터를 함수와 함께 묶어두는 방법으로, 객체지향의 인스턴스와 유사한 역할을 할 수 있습니다.

    클로저는 함수가 일급 객체인 언어에서 강력한 패턴입니다. 데코레이터도 클로저를 활용합니다.
  snippet: |-
    def outer(x):
        def inner(y):
            return x + y
    ...
    addFive(3)
  exercise:
    prompt: 클로저 정의 예제에서 함수 인자나 return 식을 바꾸고 같은 호출이 다른 값을 돌려주는지 확인하세요.
    starterCode: |-
      def outer(x):
          def inner(y):
              return x + y
  ...
  check:
    type: noError
    noError: 클로저 정의의 함수 정의, 매개변수, 호출 인자가 NameError나 TypeError 조건을 피해야 합니다.
    resultCheck: 클로저 정의 함수 호출 결과가 바꾼 인자나 반환식 기준으로 달라져야 합니다.
- id: state_encapsulation
  title: 상태 캡슐화
  structuredPrimary: true
  subtitle: 프라이빗 변수 흉내내기
  goal: 상태 캡슐화에서 추상화 패턴 흐름을 코드로 실행하고 결과를 확인한다.
  why: 함수 입력과 반환값을 작게 확인하면 이후 코드에서 같은 동작을 안전하게 재사용할 수 있습니다.
  explanation: |-
    클로저를 사용하면 외부에서 직접 접근할 수 없는 '프라이빗' 상태를 만들 수 있습니다. 외부 함수의 지역 변수는 반환된 내부 함수를 통해서만 접근하거나 수정할 수 있습니다. 이 패턴은 데이터 은닉(data hiding)을 구현하는 함수형 프로그래밍 방식입니다. 클래스의 프라이빗 속성과 유사하지만 더 간결하게 표현할 수 있습니다. 카운터, 누산기, 토글 등 간단한 상태 관리에 자주 사용됩니다.

    복잡한 상태 관리가 필요하면 클래스를 사용하세요. 클로저는 간단한 경우에 적합합니다.
  snippet: |-
    def makeCounter():
        count = 0
        def counter():
  ...
  exercise:
    prompt: 상태 캡슐화 예제에서 함수 인자나 return 식을 바꾸고 같은 호출이 다른 값을 돌려주는지 확인하세요.
    starterCode: |-
      def makeCounter():
          count = 0
          def counter():
  ...
  check:
    type: noError
    noError: 상태 캡슐화의 함수 정의, 매개변수, 호출 인자가 NameError나 TypeError 조건을 피해야 합니다.
    resultCheck: 상태 캡슐화 함수 호출 결과가 바꾼 인자나 반환식 기준으로 달라져야 합니다.
- id: factory_pattern
  title: 팩토리 패턴
  structuredPrimary: true
  subtitle: 함수 생성기
  ...
  why: 함수 입력과 반환값을 작게 확인하면 이후 코드에서 같은 동작을 안전하게 재사용할 수 있습니다.
  explanation: |-
    클로저는 특정 설정이 적용된 함수를 생성하는 팩토리로 활용됩니다. 외부 함수가 설정값을 받아 그 값이 적용된 함수를 반환합니다. 이 패턴은 비슷한 동작을 하지만 세부 설정이 다른 여러 함수를 만들 때 유용합니다. 예를 들어 특정 진법으로 변환하는 함수, 특정 형식으로 포맷팅하는 함수 등을 팩토리로 생성할 수 있습니다. 팩토리 패턴은 코드 재사용성을 높이고 설정의 분리를 가능하게 합니다.

    functools.partial도 비슷한 역할을 하지만, 클로저는 더 복잡한 로직을 캡슐화할 수 있습니다.
  snippet: |-
...
- id: memoization
  ...
  why: 함수 입력과 반환값을 작게 확인하면 이후 코드에서 같은 동작을 안전하게 재사용할 수 있습니다.
  explanation: |-
    메모이제이션(Memoization)은 함수 호출 결과를 저장하여 같은 인자로 다시 호출될 때 저장된 결과를 반환하는 최적화 기법입니다. 클로저를 사용하면 캐시를 함수와 함께 캡슐화할 수 있습니다. 재귀 함수에서 특히 효과적이며, 피보나치 수열 같은 중복 계산이 많은 알고리즘의 성능을 극적으로 향상시킵니다. 실제로는 functools.lru_cache를 사용하는 것이 더 편리하지만, 원리를 이해하는 것이 중요합니다.

    실무에서는 @functools.lru_cache를 사용하세요. 스레드 안전하고 더 많은 기능을 제공합니다.
  snippet: |-
    def memoize(func):
        cache = {}
        def wrapper(*args):
...
- id: nonlocal_keyword
  ...
  goal: nonlocal 키워드에서 추상화 패턴 흐름을 코드로 실행하고 결과를 확인한다.
  why: 함수 입력과 반환값을 작게 확인하면 이후 코드에서 같은 동작을 안전하게 재사용할 수 있습니다.
  explanation: |-
    nonlocal 키워드는 내부 함수에서 외부 함수의 지역 변수를 수정할 때 사용합니다. 파이썬에서 변수에 값을 할당하면 기본적으로 가장 가까운 스코프에 새 변수를 생성합니다. nonlocal을 사용하면 바깥 스코프의 기존 변수를 참조하여 수정할 수 있습니다. global은 모듈 수준의 전역 변수를, nonlocal은 감싸고 있는 함수의 지역 변수를 수정합니다. 클로저에서 상태를 변경해야 할 때 필수적입니다.

    nonlocal 없이도 리스트나 딕셔너리 같은 가변 객체의 내용은 수정할 수 있습니다.
  snippet: |-
    def outerBad():
        count = 0
        def inner():
...
- id: closure_vs_class
  ...
  goal: 클로저 vs 클래스에서 추상화 패턴 흐름을 코드로 실행하고 결과를 확인한다.
  why: 함수 입력과 반환값을 작게 확인하면 이후 코드에서 같은 동작을 안전하게 재사용할 수 있습니다.
  explanation: |-
    클로저와 클래스는 둘 다 상태를 캡슐화할 수 있습니다. 클로저는 단일 동작과 간단한 상태에 적합하고, 클래스는 여러 메서드와 복잡한 상태에 적합합니다. 클로저는 코드가 더 간결하고 함수형 스타일에 맞지만, 디버깅이 어려울 수 있습니다. 클래스는 명시적이고 확장성이 좋으며, 타입 힌팅과 IDE 지원이 더 좋습니다. 콜백이나 데코레이터처럼 함수를 인자로 전달해야 하는 경우에는 클로저가 자연스럽습니다.

    상태가 하나이고 동작도 하나라면 클로저, 그 외에는 클래스를 고려하세요.
  snippet: |-
    ...
    inc(), inc(), get()
  exercise:
    prompt: 클로저 vs 클래스 예제에서 함수 인자나 return 식을 바꾸고 같은 호출이 다른 값을 돌려주는지 확인하세요.
    starterCode: |-
      def makeClosureCounter():
          value = 0
          def increment():
  ...
  check:
    type: noError
    noError: 클로저 vs 클래스의 함수 정의, 매개변수, 호출 인자가 NameError나 TypeError 조건을 피해야 합니다.
    resultCheck: 클로저 vs 클래스 함수 호출 결과가 바꾼 인자나 반환식 기준으로 달라져야 합니다.
- id: workflow_validation
  title: '현업 흐름 검증: 클로저로 요청 제한기와 캐시 만들기'
  structuredPrimary: true
  subtitle: 예측 → 상태 캡슐화 → 초과 오류 → 캐시 검증
  goal: '현업 흐름 검증: 클로저로 요청 제한기와 캐시 만들기에서 추상화 패턴 흐름을 코드로 실행하고 결과를 확인한다.'
  why: 예상값과 실제 결과를 코드로 비교하면 눈으로만 확인하는 실수를 줄일 수 있습니다.
  explanation: |-
    클로저는 간단한 상태를 함수 안에 숨기고, 외부에서는 정해진 동작으로만 상태를 바꾸게 할 때 좋습니다. 요청 제한기와 계산 캐시를 만들어 상태가 기대대로 유지되는지 확인합니다.

    변주 실험
    요청 제한기를 사용자 ID별로 따로 동작하게 바꾸고, A 사용자의 초과가 B 사용자에게 영향을 주지 않는지 검증하세요.
  tips:
  - 변주 실험 요청 제한기를 사용자 ID별로 따로 동작하게 바꾸고, A 사용자의 초과가 B 사용자에게 영향을 주지 않는지 검증하세요.
  snippet: |-
  ...
  exercise:
    prompt: '현업 흐름 검증: 클로저로 요청 제한기와 캐시 만들기 예제에서 함수 인자나 return 식을 바꾸고 같은 호출이 다른 값을 돌려주는지 확인하세요.'
    starterCode: |-
      def makeRateLimiter(limit):
          if limit <= 0:
              raise ValueError("limit must be positive")
  ...
  check:
    type: noError
    noError: '현업 흐름 검증: 클로저로 요청 제한기와 캐시 만들기의 함수 정의, 매개변수, 호출 인자가 NameError나 TypeError 조건을 피해야 합니다.'
    resultCheck: '현업 흐름 검증: 클로저로 요청 제한기와 캐시 만들기 함수 호출 결과가 바꾼 인자나 반환식 기준으로 달라져야 합니다.'
- id: practice
  title: 종합 복습
  structuredPrimary: true
  ...
  why: 함수 입력과 반환값을 작게 확인하면 이후 코드에서 같은 동작을 안전하게 재사용할 수 있습니다.
  explanation: Day 3에서 배운 클로저와 상태 관리를 난이도별로 복습합니다. 클로저는 외부 함수의 변수를 기억하는 내부 함수로, 상태를 캡슐화하는 핵심 메커니즘입니다.
    nonlocal 키워드로 외부 변수를 수정하고, 이를 활용해 카운터, 캐시, 팩토리 등을 만들 수 있습니다. 🟢 기본 문제로 클로저 생성과 변수 캡처를 익히고, 🟡 응용 문제로
    상태 관리 패턴을 연습하세요. 🔴 심화 문제에서는 메모이제이션, 지연 평가, 데코레이터 구현 등 고급 활용법을 다룹니다. 클로저를 이해하면 데코레이터, 콜백, 이벤트 핸들러 등
    고급 패턴의 동작 원리가 명확해집니다.
  tips:
