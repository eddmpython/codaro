...
def buildSampleDescriptors() -> list[dict]:
    return [
        ...
        ui.radio(["a", "b"], value="a", label="r", onChange=lambda v: None),
        ui.multiselect(["a", "b"], value=["a"], label="m", onChange=lambda v: None),
        ui.date(value="2026-06-05", label="d", onChange=lambda v: None),
        ui.file(label="f", multiple=True, onChange=lambda v: None),
        ui.form(ui.text("draft"), label="form", onChange=lambda v: None),
