...
- exercise.solution: 직전 snippet 누적 namespace의 복사본에서 단독 실행.
- exercise.starterCode: 학습자가 노트북에서 실제로 실행하는 셀. blank "___" 자리표시자가 없는
  완성형이면 섹션 누적 namespace 복사본에서 실행한다. snippet/이전 snippet이 정의하지 않은 이름을
  참조하는 깨진 연습(미정의 헬퍼·집계 변수)을 NameError로 잡는다. blank 셀은 미완성이라 제외.
- blocks[].code: 섹션 보조/실습(expansion 포함)의 Python code 블록을 같은 섹션 namespace
  복사본에서 순서대로 실행. shell/browser/task/skill 언어 블록과 blank("___") 포함 starter
  셀은 제외. (base 흐름인 snippet/solution 누적 namespace는 건드리지 않는다.)
...
# code 블록 중 Python 으로 실행할 언어. 나머지(shell/browser/task/skill)는 실행하지 않는다.
_PY_BLOCK_LANGS: frozenset[str] = frozenset({"", "python", "py", "python3"})
# 학습자가 채우는 blank 자리표시자. 이 마커가 있으면 정답이 아니라 starter 셀이므로 실행 제외.
_BLANK_MARKER = "___"


def iterExecutableCodeBlocks(blocks: Any) -> list[tuple[str, str]]:
    """sections[].blocks 트리에서 실행 가능한 Python code 를 (label, code) 로 평탄화한다.

    - type==code: language 가 Python 계열이고 content 가 비어있지 않으면 채택.
    - type==expansion: 별도 `code:` 솔루션 + 중첩 `blocks` 를 재귀.
    - 그 외 type: 방어적으로 중첩 `blocks` 만 재귀(대부분 None).
    blank("___") 포함 셀은 starter 자리표시자라 호출부에서 제외한다(여기선 그대로 흘려보냄).
    """
    out: list[tuple[str, str]] = []

    def _walk(nodes: Any, trail: str) -> None:
        ...
        for idx, block in enumerate(nodes):
            ...
            if blockType == "code":
                ...
                if lang in _PY_BLOCK_LANGS and isinstance(content, str) and content.strip():
                    out.append((label, content))
            elif blockType == "expansion":
                code = block.get("code")
                if isinstance(code, str) and code.strip():
                    out.append((f"{label}.code", code))
                _walk(block.get("blocks"), f"{label}.blocks")
            else:
                _walk(block.get("blocks"), f"{label}.blocks")
...
# 실행 환경에서만(자격 증명/네트워크 미준비) 나는 실패 신호 — 콘텐츠 결함이 아니다.
# 예: anthropic SDK 는 ANTHROPIC_API_KEY 가 없으면 messages.create 에서 TypeError 를 던진다.
# 이런 신호는 "환경 미준비를 제품 결함으로 오판"하지 않도록 runtime-other(정보성)로 분류한다.
_ENV_CREDENTIAL_MARKERS: tuple[str, ...] = (
    "Could not resolve authentication method",
    "ANTHROPIC_API_KEY",
)


def isEnvCredentialError(detail: str) -> bool:
    return any(marker in detail for marker in _ENV_CREDENTIAL_MARKERS)
...
def auditLesson(path: Path) -> list[dict[str, Any]]:
    ...
    # 읽기/오리엔테이션 레슨(meta.badge == "읽기")의 code 블록은 실행 계약이 아니라
    # 설명용 단편(예: `run.font.name = ...`처럼 문맥상 존재를 가정한 객체 참조)이다.
    # weakness audit가 같은 이유로 읽기 레슨의 exercise를 면제하듯, 블록 실행도 제외한다.
    metaBlock = data.get("meta") if isinstance(data.get("meta"), dict) else {}
    isReadingLesson = str(metaBlock.get("badge") or "").strip() == "읽기"
    ...
    for section in sections:
        ...
        # 정의하지 않은 이름을 참조하는 깨진 연습(예: 미정의 헬퍼 fitRiskModel·집계 변수
        # revenueByChannel)을 NameError 로 잡는다. solution 만 보는 base audit 의 사각지대였다.
        # blank 셀은 학습자가 채우는 미완성 코드라 실행 대상이 아니다(solution 이 정답 계약).
        ...
        if isinstance(starterCode, str) and starterCode.strip() and _BLANK_MARKER not in starterCode:
            starterNs = dict(namespace)
            category, detail = execCode(starterCode, starterNs, f"{relPath}::{sectionId}.starterCode")
            if category in ("real-bug", "runtime-other") and declaredMissingModule(detail, declaredPackages):
                category = "missing-package"
            if category == "real-bug" and isEnvCredentialError(detail):
                category = "runtime-other"
            ...
            if category == "missing-package":
                ...
                if pkgName and pkgName not in declaredPackages:
                    category = "undeclared-package"
            results.append({"path": relPath, "section": sectionId, "kind": "starterCode",
                            "category": category, "detail": detail})

        # blocks[].code: 보조/실습 code 블록 (base audit 사각지대). 섹션 namespace 복사본에서
        # 순서대로 실행하므로 base snippet/solution 누적 흐름은 그대로 둔다.
        sectionBlocks = section.get("blocks")
        if isinstance(sectionBlocks, list) and not isReadingLesson:
            blockNs = dict(namespace)
            for blockLabel, blockCode in iterExecutableCodeBlocks(sectionBlocks):
                if _BLANK_MARKER in blockCode:
                    continue
                category, detail = execCode(blockCode, blockNs, f"{relPath}::{sectionId}.{blockLabel}")
                if category in ("real-bug", "runtime-other") and declaredMissingModule(detail, declaredPackages):
                    category = "missing-package"
                if category == "real-bug" and isEnvCredentialError(detail):
                    category = "runtime-other"
                ...
                if category != "ok":
                    priorFailureSeen = True
                results.append({"path": relPath, "section": sectionId, "kind": blockLabel,
                                "category": category, "detail": detail})
    return results
