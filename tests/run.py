...
GATE_ARTIFACTS: dict[str, tuple[str, ...]] = {
    ...
    "pyproc-asgi-browser": ("output/test-runner/pyproc-asgi-browser/pyproc-asgi-report.json",),
...
GATES: dict[str, Gate] = {
    ...
    "pyproc-asgi-browser": Gate(
        tier="surface",
        description="editor build의 브라우저 pyproc AsgiServer가 소켓 없이 Python 요청/응답을 dispatch하는지 확인한다.",
        commands=(
            command(("npm", "run", "build"), cwd="editor"),
            command(("uv", "run", "python", "-X", "utf8", "tests/surface/verifyPyprocAsgiPlaywright.py")),
        ),
        ci_required=False,
    ),
    ...
    "static-publication": Gate(
        tier="surface",
        description="정적 app bundle의 재현성, 무결성, 상대 자산, offline Python과 위젯 반응성을 검증한다.",
        commands=(
            command((
                "uv", "run", "python", "-X", "utf8", "-m", "pytest",
    ...
    "learning-product-bridge": Gate(
        tier="surface",
        description="strong learning evidence가 동일 소스 기능 블록, 의미 검증 Task, operational proof로 이어지는지 확인한다.",
        commands=(
            command((
                "uv", "run", "python", "-X", "utf8", "-m", "pytest",
